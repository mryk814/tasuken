...
export function buildTaskAiRequest(tasks: RequestTask[]): string {
  return [
    ...
    "3. start_task_workが返した最新のTask versionを以後のexpected_versionに使ってください。進捗は tasken.append_work_receipt、完了候補は tasken.report_task_done、中断は tasken.report_task_blocked で、結果・検証・残作業を報告してください。",
