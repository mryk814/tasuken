...
import {
  ...
  TIMELINE_ITEM_STATE_LABELS,
  TIMELINE_ITEM_STATE_MARKS,
...
export function assignGanttSubLanes(items: Item[]): Map<string, number> {
  ...
  const scheduled = items
    ...
    .filter(
      (entry): entry is { item: Item; index: number; range: { start: string; end: string } } =>
        Boolean(entry.range),
    )
    .sort(
      (a, b) =>
        a.range.start.localeCompare(b.range.start) ||
        a.range.end.localeCompare(b.range.end) ||
        a.index - b.index,
    );
...
export function GanttItemRow({
  ...
  const [drag, setDrag] = useState<{ itemId: string; mode: DragMode; dxPercent: number } | null>(
    null,
  );
  ...
  function beginDrag(event: React.PointerEvent, target: Item, mode: DragMode) {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      if (Math.abs(dxPercent) > 0.5 || Math.abs(moveEvent.clientY - initialY) > 6)
  ...
  function beginRangeCreate(event: React.PointerEvent<HTMLDivElement>) {
    ...
    const startPercent = Math.max(
      ...
      Math.min(
        100,
        ((initialX - event.currentTarget.getBoundingClientRect().left) / trackWidth) * 100,
      ),
    );
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      const nextStart = Math.max(
        0,
        Math.min(100, dxPercent < 0 ? startPercent + dxPercent : startPercent),
      );
    ...
    const onPointerUp = (upEvent: PointerEvent) => {
      ...
      const startDays = Math.round(
        (Math.min(startPercent, startPercent + dxPercent) / 100) * total,
      );
      const endDays = Math.round((Math.max(startPercent, startPercent + dxPercent) / 100) * total);
      onCreateRange(
        item,
        addDays(range.start, startDays),
        addDays(range.start, Math.max(startDays, endDays)),
      );
    };
  ...
  return (
    <div
      className={`gantt-item-row level-${itemLevel(item)}`}
      ref={rowRef}
      onPointerDown={beginRangeCreate}
    ...
    >
      ...
      {bars.map((barItem) => {
        ...
        const left = start
          ? Math.max(0, Math.min(100, (daysBetween(range.start, start) / total) * 100))
          : 0;
        const width = start
          ? Math.max(
              barItem.kind === "milestone" ? 0.8 : 1.4,
              Math.min(100 - left, ((daysBetween(start, end || start) + 1) / total) * 100),
            )
          : 0;
        ...
        if (drag?.itemId === barItem.id) {
          if (drag.mode === "move") displayLeft = left + drag.dxPercent;
            ...
            displayLeft = left + drag.dxPercent;
            displayWidth = width - drag.dxPercent;
        ...
        const stateMark =
          scheduleKind === "execution_window"
            ...
            : scheduleKind === "ongoing_period"
              ? "→"
              : TIMELINE_ITEM_STATE_MARKS[state];
        const barClass = [
          ...
          <button
            ...
            style={
              {
                ...
                "--bar-color": themeColorKey ? `var(--color-${themeColorKey})` : undefined,
            ...
            title={
              isConnecting
                ? isSource
                  ? `${barItem.title}（選択中）`
                  : `${barItem.title} を後続にする`
                : `${barItem.title}\n状態: ${stateLabel}${scheduleLabel ? `\n範囲: ${scheduleLabel}` : ""}\n${hint(barItem)}\nCtrl+クリックで依存を接続`
            }
          >
            {barItem.kind !== "milestone" && !isConnecting && (
              <span
                className="resize-handle start"
                onPointerDown={(event) => beginDrag(event, barItem, "start")}
              />
            )}
            <span>
              {barItem.kind === "milestone" ? (
                ...
                <>
                  {stateMark && (
                    <span className="gantt-bar-state-mark" aria-hidden="true">
                      {stateMark}
                    </span>
                  )}
                  {barItem.title}
            ...
            {barItem.kind !== "milestone" && !isConnecting && (
              <span
                ...
                onPointerDown={(event) => beginDrag(event, barItem, "end")}
              />
            )}
          </button>
        ) : null;
...
export function TimeAxis({
  ...
  return (
    <div className="gantt-axis">
      {marks.map((mark) => (
        <span
          className={`gantt-axis-mark ${mark.kind}`}
          key={mark.date}
...
export function MilestoneLane({
  ...
  function dateFromClientX(clientX: number): string | null {
    ...
    const offset = Math.max(
      0,
      Math.min(total, Math.round(((clientX - rect.left) / Math.max(1, rect.width)) * total)),
    );
  ...
  return (
    <div
      className="gantt-milestone-lane"
      ref={laneRef}
      onPointerUp={createMilestoneAt}
      title="空白をクリックしてマイルストーンを追加"
    >
      {marks.map((mark) => {
        if (mark.date < range.start || mark.date > range.end) return null;
        const baseLeft = Math.max(
          0,
          Math.min(100, (daysBetween(range.start, mark.date) / total) * 100),
        );
        ...
        return (
          <button
            ...
            onClick={() => {
              if (!movedRef.current) onOpen(first);
              movedRef.current = false;
            }}
            onPointerDown={
              clustered ? (event) => event.stopPropagation() : (event) => beginDrag(event, first)
            }
            title={title}
          >
...
export function DependencyOverlay({
  ...
  const rowIndexOf = (id?: string) =>
    rows.findIndex(
      (row) =>
        row.rowType === "item" &&
        (row.item.id === id || row.laneItems.some((item) => item.id === id)),
    );
  const lines = dependencies.flatMap((dependency) => {
    ...
    const source =
      sourceRow.item.id === sourceItemId
        ? sourceRow.item
        : sourceRow.laneItems.find((item) => item.id === sourceItemId) || sourceRow.item;
    const target =
      targetRow.item.id === targetItemId
        ? targetRow.item
        : targetRow.laneItems.find((item) => item.id === targetItemId) || targetRow.item;
    const sourceDate = source.planned_end;
    const targetDate = target.planned_start || target.planned_end;
    if (!sourceDate || !targetDate) return [];
    if (
      sourceDate < range.start ||
      sourceDate > range.end ||
    ...
    const sourceX = Math.max(
      0,
      Math.min(100, ((daysBetween(range.start, sourceDate) + 1) / total) * 100),
    );
    const targetX = Math.max(
      0,
      Math.min(100, (daysBetween(range.start, targetDate) / total) * 100),
    );
    ...
    const laneClearance = Math.min(4, Math.max(dayPercent * 2, 0.9));
    const hasForwardSpace = targetX - sourceX >= laneClearance * 2;
    // 依存線は「出発点の右 → 専用の縦レーン → 到着点」の直交経路に統一する。
    // 逆向き・重なりは右側の空きレーンを通すため、バーを斜めに横切らない。
    const bendX = hasForwardSpace
      ? sourceX + (targetX - sourceX) / 2
      : Math.min(99.4, Math.max(sourceX, targetX) + laneClearance);
    return [
      {
        id: dependency.id,
        sourceX,
...
export function LightningOverlay({
  ...
  const points = rows.flatMap((row, index) => {
    ...
    // 実績が未入力の進行中計画は、今日時点では予定どおりとして描く。
    // 終了予定を越えた未完了項目だけは status 由来の遅れを明示する。
    const isOnSchedule = timelineItemState(item, today) === "ongoing";
    const actualProgress =
      isInProgressStatus(item.status) || isOnSchedule
        ? plannedProgress
    ...
    return [
      { x, y: BASE_ROW_HEIGHT + top + ganttBarCenterY(rowItems(row), item.id), varianceDays },
    ];
  });
  if (!points.length) return null;
  const height = Math.max(
    88,
    rows.reduce((sum, row) => sum + timelineRowHeight(row), BASE_ROW_HEIGHT),
  );
