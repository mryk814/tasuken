...
export function findSchedule(
  ...
  return (data.schedules || []).find(
    (schedule) => (schedule as unknown as Schedule).owner_type === ownerType
...
export function TaskFields({
  ...
  const repeatRule = entity.repeat_rule && typeof entity.repeat_rule === "object"
  ...
  return (
    <>
      ...
      <label className="toggle priority-toggle">
        <input name="priority_flag" type="checkbox" defaultChecked={str(entity.priority) === "high"} />
        旗を付ける
      </label>
...
export function WaitingFields({ entity, data }: { entity: DrawerConfig["entity"]; data: WorkspaceData }) {
  ...
  return (
    <>
      ...
      <Field label="確認リマインダー">
        <input name="check_reminder_at" type="datetime-local" defaultValue={normalizeReminderDateTime(entity.check_reminder_at) || ""} />
      </Field>
...
export function PlanNodeFields({ entity, data }: { entity: DrawerConfig["entity"]; data: WorkspaceData }) {
  ...
  const [dateUnit, setDateUnit] = useState(
    str(schedule?.granularity) === "month" || str(entity.schedule_granularity) === "month" ? "month" : "day",
  );
  ...
  return (
    <>
      ...
      {showMilestoneDate && (
        <Field label="日付"><input name="start_date" type="date" defaultValue={milestoneDate} /></Field>
      )}
      <Field label="説明"><textarea name="description" defaultValue={str(entity.description)} /></Field>
