...
const normalizeReferenceStatus = (value: unknown) =>
  str(value) === "adopted" ? "adopted" : "inbox";
...
function DerivedSourceReference({
  ...
  const reference = ((data.references || []) as unknown as Reference[]).find(
    (entry) =>
...
function TaskWorkSection({
  ...
  const run = async (
    ...
    try {
      ...
      setToast(
        `Work stateを更新できませんでした。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    } finally {
      setBusy(false);
      ...
      <div className="drawer-disclosure-body">
        ...
        {workState === "in_progress" && (
          <form className="form-grid" onSubmit={report}>
            ...
            <Field label="変更・作成した項目">
              <textarea
                value={changedItems}
                onChange={(event) => setChangedItems(event.target.value)}
                placeholder="1行1項目"
              />
            </Field>
            <Field label="検証">
...
export function EntityDrawer({
  ...
  if (type === "knowledge_node")
    return <KnowledgeNodeDetailDrawer node={entity as KnowledgeNode} data={data} close={close} />;
  if (type === "knowledge_edge")
    return (
      <KnowledgeEdgeDetailDrawer
        edge={entity as unknown as import("../domain-model/types").KnowledgeEdge}
        data={data}
        close={close}
      />
    );
      ...
      >
        <div className="badge-row">
          <StatusBadge
            value="neutral"
            label={
              sketchCanvasMode(sketch.document) === "infinite"
                ? "Infinite Canvas"
                : `${sketch.document.pages.length}ページ`
            }
          />
          ...
          <ThemePickerSelect
            ...
            onChange={async (next) => {
              try {
                ...
                setToast(
                  `Themeを更新できませんでした。もう一度選択してください。${error instanceof Error ? ` ${error.message}` : ""}`,
                  "danger",
                );
              }
            }}
        ...
        <dl>
          <dt>作成</dt>
          <dd>{formatDate(sketch.created_at)}</dd>
          <dt>更新</dt>
          <dd>{formatDate(sketch.updated_at)}</dd>
        </dl>
        <LineagePanel
          data={data}
  ...
  if (type === "resource") {
    ...
    const themeName =
      (data.themes || []).find((t) => t.id === (entity.project_id || entity.theme_id))?.name ||
      "未設定";
    ...
    const relatedTasks = ((data.references || []) as unknown as Reference[])
      ...
      .map((reference) =>
        reference.source_type === "task" ? reference.source_id : reference.target_id,
      )
      ...
      >
        {isChatRef && (
          <div className="badge-row">
            ...
            <StatusBadge
              value={normalizeReferenceStatus(entity.reference_status)}
              label={
                CHAT_REFERENCE_STATUS_LABELS[normalizeReferenceStatus(entity.reference_status)]
              }
            />
        ...
        {Boolean(entity.url) && (
          <a href={str(entity.url)} target="_blank" rel="noreferrer">
            {str(entity.url)}
          </a>
        )}
        <dl>
          ...
          {isChatRef && (
            <>
              <dt>グループ</dt>
              <dd>{str(entity.chat_group) || "未分類"}</dd>
                    ...
                    <button
                      ...
                      onClick={() =>
                        close({
                          ...
                          entity: {
                            ...task,
                            _schedule: findSchedule(data, "task", task.id),
        ...
        {Boolean(entity.body_markdown) &&
          (isChatRef && isConversationMarkdown(str(entity.body_markdown)) ? (
            ...
            <MarkdownPreview
              ...
              html={previewHtml(str(entity.body_markdown), "markdown")}
            />
          ))}
        <LineagePanel
          data={data}
  ...
  if (type === "task") {
    ...
    const learningNotes = (data.notes || [])
      ...
      .sort((a, b) =>
        str(b.created_at || b.updated_at).localeCompare(str(a.created_at || a.updated_at)),
      );
    const copyTask = async () => {
      ...
      close({
        type: "task",
        mode: "edit",
        entity: { ...duplicated.task, _schedule: duplicated.schedule } as Record<string, unknown>,
      });
    };
        ...
        <div className="drawer-content">
                  ...
                  <li key={item.id} className={item.done ? "is-done" : ""}>
                    <label>
                      <input
                        ...
                        checked={item.done}
                        onChange={async () => {
                          const nextItems = (task.checklist_items || []).map((entry) =>
                            entry.id === item.id
                              ? {
                                  ...entry,
                                  done: !entry.done,
          ...
          <details className="drawer-subsection drawer-disclosure task-related-disclosure">
            ...
            <div className="drawer-disclosure-body">
              ...
              <section className="task-learning-section">
                <div className="section-heading">
                  <h3>気づき・学び</h3>
                  <button className="text-button compact" onClick={() => addLearningNote(false)}>
                    追加
                  </button>
                </div>
          ...
          <div className="drawer-actions">
            ...
            <button
              className="secondary-button"
              onClick={() =>
                close({ type: "task", mode: "edit", entity: { ...entity, _schedule: schedule } })
              }
            >
              <IconPencil size={16} />
              編集する
            </button>
            <button className="secondary-button" onClick={copyTask}>
            ...
            <button
              ...
              title={
                requiresHumanAcceptance ? "AIの報告を人間が確認してから完了できます。" : undefined
              }
              onClick={async () => {
            ...
            {task.state !== "done" && (
              <button
                className="secondary-button"
                disabled={requiresHumanAcceptance}
                onClick={() => addLearningNote(true)}
              >
                完了して学びを書く
              </button>
            )}
            <button className="danger-button" onClick={() => removeEntity("task", entity)}>
              <IconTrash size={16} />
              削除する
            </button>
          </div>
  ...
  if (type === "waiting") {
    ...
    const themeName =
      (data.themes || []).find((t) => t.id === waiting.project_id)?.name || "個人業務";
    return (
      <aside className="drawer">
        ...
        <div className="drawer-content">
          ...
          <dl>
            <dt>相手</dt>
            <dd>{waiting.waiting_for}</dd>
            <dt>Theme</dt>
            <dd>{themeName}</dd>
            <dt>期限</dt>
            <dd>{formatDate(schedule?.end_date)}</dd>
          </dl>
          ...
          <div className="drawer-actions">
            <button
              className="secondary-button"
              onClick={() =>
                close({ type: "waiting", mode: "edit", entity: { ...entity, _schedule: schedule } })
              }
            >
              <IconPencil size={16} />
              編集する
            ...
            {waiting.state === "waiting" ? (
              <>
                <button
                  ...
                  onClick={async () => {
                    await saveEntities(
                      buildSaveWaitingOperations({ ...waiting, state: "received" }),
                      "受領しました。",
                    );
                    close();
                  }}
                ...
                <button
                  ...
                  onClick={async () => {
                    await saveEntities(
                      buildSaveWaitingOperations({ ...waiting, state: "cancelled" }),
                      "中止しました。",
                    );
                    close();
                  }}
              ...
              <button
                ...
                onClick={async () => {
                  await saveEntities(
                    buildSaveWaitingOperations({ ...waiting, state: "waiting" }),
                    "待ちに戻しました。",
                  );
                  close();
                }}
            ...
            <button className="danger-button" onClick={() => removeEntity("waiting", entity)}>
              <IconTrash size={16} />
              削除する
            </button>
          </div>
  ...
  if (type === "plan_node") {
    ...
    const themeName =
      (data.themes || []).find((t) => t.id === planNode.project_id)?.name || "個人業務";
    return (
      <aside className="drawer">
          ...
          <div className="drawer-actions">
            <button
              ...
              onClick={() =>
                close({
                  type: "plan_node",
                  mode: "edit",
                  entity: { ...entity, _schedule: schedule },
                })
              }
            >
              <IconPencil size={16} />
            ...
            <button
              ...
              onClick={async () => {
                ...
                await saveEntities(
                  buildSavePlanNodeOperations({ ...planNode, state: nextState }),
                  nextState === "done" ? "完了しました。" : "未完了に戻しました。",
                );
                close();
              }}
            >
              {planNode.state === "done" ? "未完了に戻す" : "完了にする"}
            </button>
            <button className="danger-button" onClick={() => removeEntity("plan_node", entity)}>
              <IconTrash size={16} />
              削除する
            </button>
          </div>
      ...
      <aside className="drawer">
        ...
        <div className="drawer-content">
          ...
          <div className="drawer-actions">
            ...
            <button className="danger-button" onClick={() => removeEntity("capture_entry", entity)}>
              <IconTrash size={16} />
              削除する
            </button>
          </div>
  ...
  const artifactSource = (() => {
    ...
    if (type === "task") {
      return {
        ...
        sourceId: entityId,
        themeId: str(entity.project_id) || null,
      };
    }
    if (type === "note") {
...
function NoteFields({ entity, data }: { entity: DrawerConfig["entity"]; data: WorkspaceData }) {
  ...
  const properties =
    entity.properties_json && typeof entity.properties_json === "object"
  ...
  const initialHeadingLevels = Object.prototype.hasOwnProperty.call(
    properties,
    "heading_number_levels",
  )
  ...
  return (
    <>
      ...
      <ThemeSelect
        themes={data.themes}
        value={str(entity.project_id ?? entity.theme_id)}
        allowPersonal
      />
      <Field label="種別">
        <select
          name="note_type"
          value={noteType}
          onChange={(event) => chooseNoteType(event.target.value)}
        >
      ...
      {isReport && (
        <div className="form-grid">
          ...
          <Field label="対象開始">
            <input name="period_start" type="date" defaultValue={str(properties.period_start)} />
          </Field>
          <Field label="対象終了">
        ...
        <Field label="出力設定">
          ...
          <label className="toggle">
            <input
              ...
              value="true"
              defaultChecked={
                properties.publish_enabled === true || properties.export_enabled === true
              }
            />
      ...
      {!isPrompt && contentFormat === "markdown" && (
        <Field label="見出し番号">
          ...
          <label className="toggle">
            <input
              type="checkbox"
              name="heading_numbers"
              value="true"
              defaultChecked={properties.heading_numbers === true}
            />
...
function ResourceFields({ entity, data }: { entity: DrawerConfig["entity"]; data: WorkspaceData }) {
  ...
  return (
    <>
      ...
      <Field label="URL">
        <input name="url" type="url" value={url} onChange={(event) => setUrl(event.target.value)} />
      </Field>
      <ThemeSelect
        themes={data.themes}
        value={projectId}
          ...
          <div className="form-grid">
            <Field label="サービス">
              ...
              >
                ...
                {CHAT_SERVICE_TYPES.map((value) => (
                  <option key={value} value={value}>
                    {CHAT_SERVICE_LABELS[value]}
                  </option>
                ))}
            ...
            <Field label="参照状態">
              <select
                ...
                defaultValue={normalizeReferenceStatus(entity.reference_status)}
              >
                {CHAT_REFERENCE_STATUSES.map((value) => (
                  <option key={value} value={value}>
                    {CHAT_REFERENCE_STATUS_LABELS[value]}
                  </option>
                ))}
              </select>
            </Field>
            <Field label="保存日">
              <input
                name="captured_at"
                type="date"
                defaultValue={chatDateInput(entity.captured_at || entity.created_at)}
              />
            </Field>
...
function KnowledgeNodeFields({
  ...
  return (
    <>
      <Field label="種類">
        <select name="node_type" defaultValue={selectedNodeType}>
          {nodeTypeOptions.map((value) => (
            <option key={value} value={value}>
              {KNOWLEDGE_NODE_LABELS[value] || value}
            </option>
          ))}
        </select>
      </Field>
      ...
      <Field label="本文">
        <textarea className="large-textarea" name="body" defaultValue={str(entity.body)} />
      </Field>
    ...
    <>
      ...
      <Field label="関係種別">
        <select name="relation_type" defaultValue={str(entity.relation_type) || "supports"}>
          {Object.entries(KNOWLEDGE_RELATION_LABELS).map(([value, label]) => (
            <option key={value} value={value}>
              {label}
            </option>
          ))}
  ...
  return (
    <aside className="drawer">
      ...
      <div className="drawer-content">
        ...
        <div className="drawer-actions">
          {onOpenCanvas && (
            <button className="secondary-button" onClick={onOpenCanvas}>
...
function NoteDetailDrawer({
  ...
  const properties =
    note.properties_json && typeof note.properties_json === "object"
  ...
  const headingNumberLevelSummary = headingNumberLevels.length
    ? headingNumberLevels.map((level) => `h${level}`).join("–")
    : "選択なし";
  ...
  const markdownExport =
    properties.markdown_export &&
    typeof properties.markdown_export === "object" &&
    !Array.isArray(properties.markdown_export)
  ...
  const markdownExportStale = Boolean(
    exportedSignature && exportedSignature !== currentExportSignature,
  );
  ...
  const publishMarkdownBody = [
    ...
    str(note.updated_at || note.created_at)
      ? `updated_at: ${JSON.stringify(str(note.updated_at || note.created_at))}`
      : "",
    "---",
    "",
    body.trim(),
  ...
  const emailBodyHtml = [
    ...
    periodLabel
      ? `<p style="margin:0 0 12px;color:#666;">対象期間: ${escapeHtml(periodLabel)}</p>`
      : "",
  ...
  async function updateHeadingNumberSettings(patch: {
    ...
    const nextEnabled =
      patch.heading_numbers ?? headingNumberOptions.preview.headingNumbers === true;
    ...
    const saved = await saveEntity("note", {
      ...
      properties_json: {
        ...
        heading_number_start:
          nextLevels[0] ??
          normalizeHeadingNumberStart(headingNumberOptions.preview.headingNumberStart),
      },
    });
    if (patch.heading_numbers !== undefined && patch.heading_number_levels === undefined) {
      setToast(
        nextEnabled
          ? "見出し番号を表示します（Preview / PDF）。"
          : "見出し番号を非表示にしました。",
        "success",
      );
  ...
  async function exportMarkdown(chooseDirectory: boolean) {
    ...
    try {
      ...
      setToast(
        `Markdown出力に失敗しました。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    } finally {
  ...
  return (
    <aside className="drawer">
      ...
      <div className="drawer-content">
        <StatusBadge
          value="neutral"
          label={NOTE_TYPE_LABELS[note.note_type ?? ""] || note.note_type}
        />
        <h2>{note.title}</h2>
        <DerivedSourceReference
          data={data}
        ...
        />
        <section
          className={`document-rule-strip ${publishEnabled ? "is-export-target" : "is-export-muted"}`}
        >
          <div>
            <strong>{publishEnabled ? "一括出力対象" : "一括出力対象外"}</strong>
            <span>
              {publishEnabled
                ? "Markdown / PDF 一括出力の対象です。"
                : "文書出力の対象から外れています。"}
            </span>
          </div>
          <label className="toggle">
        ...
        {contentFormat === "markdown" && (
          <section className="document-rule-strip">
            <div>
              <strong>見出し番号</strong>
              <span>
                本文は書き換えず、Preview / PDF に通し番号を付けます。対象レベルだけ番号になります。
              </span>
            </div>
            <div className="document-publish-actions">
              <label className="toggle note-heading-number-toggle">
                <input
                  ...
                  onChange={(event) =>
                    updateHeadingNumberSettings({ heading_numbers: event.target.checked })
                  }
                />
                表示する
              </label>
          ...
          <section className="artifact-preview-section">
            <div className="section-heading">
              <div className="segmented" aria-label="Markdown表示">
                <button
                  className={artifactMode === "preview" ? "is-active" : ""}
                  onClick={() => setArtifactMode("preview")}
                >
                  Preview
                </button>
                <button
                  className={artifactMode === "raw" ? "is-active" : ""}
              ...
              <button
                className="secondary-button compact"
                onClick={() => workspaceApi.copyText(body)}
              >
                Rawをコピー
              </button>
              <button
                className="secondary-button compact"
                onClick={() => workspaceApi.copyText(renderedText(body, contentFormat))}
              >
                Previewをコピー
              </button>
            </div>
        ...
        {isReport && (
          <section className="report-email-panel">
            ...
            <div className="document-publish-actions">
              <button
                className="secondary-button compact"
                onClick={() => copyReportEmail("subject")}
              >
                件名をコピー
              </button>
              <button className="secondary-button compact" onClick={() => copyReportEmail("body")}>
                Outlook本文をコピー
              </button>
              <button
                className="primary-button compact"
                onClick={() => copyReportEmail("combined")}
              >
                件名+本文をコピー
              </button>
            </div>
        ...
        {canExportDocument && (
          <section
            className={`document-publish-panel ${markdownExportStale ? "needs-export" : ""}`}
          >
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (result.ok) setToast("Markdownの保存先フォルダを開きました。", "success");
                      ...
                      setToast(
                        result.error || "Markdownの保存先フォルダを開けませんでした。",
                        "danger",
                      );
                  }}
                >
              ...
              {comments.map((entry) => (
                <div className="comment-item" key={entry.id}>
                  ...
                  <div className="comment-meta">
                    ...
                    <button className="text-button compact" onClick={() => removeComment(entry.id)}>
                      削除する
                    </button>
                  </div>
                </div>
        ...
        <div className="drawer-actions">
          <button
            className="primary-button"
            onClick={() => close({ type: "note", mode: "edit", entity: note })}
          >
            <IconPencil size={16} />
            編集する
          </button>
          <button className="danger-button" onClick={() => removeEntity("note", note)}>
            <IconTrash size={16} />
            削除する
          </button>
        </div>
...
function KnowledgeNodeDetailDrawer({
  ...
  const relations = (
    (data.knowledge_edges || []) as unknown as import("../domain-model/types").KnowledgeEdge[]
  ).filter(
    (relation) => relation.source_node_id === node.id || relation.target_node_id === node.id,
  );
  ...
  const allResources = [
    ...(data.resources || []),
    ...(data.links || []).filter((l) => !resourceIds.has(l.id)),
  ];
  ...
  const sourceLabel = (() => {
    if (node.source_type && node.source_id) {
      ...
      if (node.source_type === "note") {
        const n = data.notes.find((n) => n.id === node.source_id);
        return `${typeLabel}: ${n?.title || node.source_id}`;
      }
      if (node.source_type === "resource") {
        const r = allResources.find((r) => r.id === node.source_id);
        return `${typeLabel}: ${str(r?.title) || node.source_id}`;
      }
      const t = allTasks.find((t) => t.id === node.source_id);
      return `${typeLabel}: ${str(t?.title) || node.source_id}`;
    }
    if (node.source_note_id)
      return `メモ: ${data.notes.find((n) => n.id === node.source_note_id)?.title || "不明"}`;
    if (node.source_link_id)
      return `リソース: ${allResources.find((r) => r.id === node.source_link_id)?.title || "不明"}`;
    if (node.source_item_id)
      return `タスク: ${allTasks.find((t) => t.id === node.source_item_id)?.title || "不明"}`;
    return "未設定";
  })();
    ...
    <aside className="drawer">
      ...
      <div className="drawer-content">
        <div className="badge-row">
          <StatusBadge
            value={node.status}
            label={KNOWLEDGE_NODE_LABELS[node.node_type] || node.node_type}
          />
        ...
        <dl>
          <dt>Theme</dt>
          <dd>{data.themes.find((theme) => theme.id === node.theme_id)?.name || "未設定"}</dd>
          <dt>出典</dt>
          <dd>{sourceLabel}</dd>
        </dl>
        {relations.length > 0 && (
          <div className="revision-list">
            ...
            {relations.map((relation) => {
              ...
              const other = data.knowledge_nodes.find(
                (entry) =>
                  entry.id === (isSource ? relation.target_node_id : relation.source_node_id),
              );
              return (
                <div key={relation.id}>
                  <span>
                    {isSource ? "→" : "←"} {relation.relation_type}: {str(other?.title) || "不明"}
                  </span>
                </div>
              );
            })}
            ...
            {linkContext.backlinks.map((entry) => (
              <div key={`${entry.type}-${entry.id}`}>
                <div className="knowledge-link-row">
                  ...
                  <span>
                    {entry.type === "note" ? "Note" : "Knowledge"} /{" "}
                    {entry.body.slice(0, 80) || "本文なし"}
                  </span>
                </div>
              </div>
            ))}
  ...
  return (
    <aside className="drawer">
      ...
      <div className="drawer-content">
        ...
        <dl>
          <dt>関係元</dt>
          <dd>{source?.title || edge.source_node_id || "不明"}</dd>
          <dt>関係種別</dt>
          <dd>{KNOWLEDGE_RELATION_LABELS[edge.relation_type] || edge.relation_type || "未設定"}</dd>
          <dt>関係先</dt>
          <dd>{target?.title || edge.target_node_id || "不明"}</dd>
        </dl>
        {edge.description && <p className="note-body">{edge.description}</p>}
        <p className="field-help">
          既存Relationの確認専用です。この画面からの追加・編集・削除は行いません。
        </p>
      </div>
    </aside>
  );
