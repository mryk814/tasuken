    ...
    <div className={`repository-context-row${isArchived ? " is-archived" : ""}`}>
      {!isArchived ? (
        <label className="toggle repository-context-option">
          ...
          <span>
            ...
            <small>
              {REPOSITORY_PROVIDER_LABELS[context.provider] || "Unknown"} · {contextTarget(context)}
            </small>
          </span>
        </label>
      ) : (
        <span className="repository-context-option">
          <span>
            <strong>{contextLabel(context)}</strong>
            <small>Archive · {contextTarget(context)}</small>
          </span>
        </span>
      )}
      <div className="repository-context-row-actions">
        ...
        {isArchived && onRestore && (
          <button type="button" className="text-button compact" onClick={() => void onRestore()}>
            復元
          </button>
        )}
        {!isArchived && onArchive && (
          <button type="button" className="text-button compact" onClick={() => void onArchive()}>
            Archive
          </button>
        )}
        {!isArchived && onDelete && (
      ...
      {editing && (
        <div className="repository-context-edit form-grid">
          <Field label="Label">
          ...
          <button type="button" className="secondary-button compact" onClick={() => void save()}>
            保存
          </button>
        </div>
      )}
...
function ContextOptions({
  ...
  if (!contexts.length)
    return <p className="field-help">登録済みのRepositoryContextはありません。</p>;
  return (
...
function PrimaryContextSelect({
  ...
  return (
    <Field label="Primary RepositoryContext">
      <select
        name="primary_repository_context_id"
        value={value}
        onChange={(event) => onChange(event.target.value)}
      >
        {contexts.map((context) => (
          <option key={String(context.id)} value={String(context.id)}>
      ...
      [
        {
          ...
          entity: context as unknown as SaveOperation["entity"],
        },
      ],
      "RepositoryContextを保存しました。",
    );
  };
  const archiveContext = async (context: RepositoryContext) =>
    saveContext({ ...context, active: false });
  const restoreContext = async (context: RepositoryContext) =>
    saveContext({ ...context, active: true, deleted_at: null });
  const deleteContext = async (context: RepositoryContext) =>
    removeEntity?.("repository_context", context as unknown as DrawerEntity);
...
export function ThemeRepositoryContextFields({
  ...
  const archivedContexts = allContexts.filter(
    (context) => !context.deleted_at && context.active === false,
  );
  const initialIds = Array.isArray(entity.repository_context_ids)
    ? entity.repository_context_ids.map(String)
    : [];
  const [selectedIds, setSelectedIds] = useState(
    initialIds.filter((id) => contexts.some((context) => String(context.id) === id)),
  );
  ...
  const missingIds = initialIds.filter(
    (id) => !contexts.some((context) => String(context.id) === id),
  );
  ...
  return (
    ...
    >
      <div className="section-heading">
        <h2>Repository</h2>
        <span className="field-help">このThemeで使うRepositoryを登録・選択します。</span>
      </div>
      <div className="repository-context-options">
        {contexts.length ? (
          contexts.map((context) => (
            <ContextRow
              ...
              onDelete={async () => {
                await actions.deleteContext(context);
                removeSelection(String(context.id));
              }}
              onArchive={async () => {
                await actions.archiveContext(context);
                removeSelection(String(context.id));
              }}
            />
          ))
      ...
      {missingIds.map((id) => (
        <p className="field-help" key={id}>
          参照 {id} は利用できません（削除済み・Archive・未登録の可能性）。
        </p>
      ))}
      {selectedIds.length > 0 && (
        <PrimaryContextSelect contexts={contexts} value={primaryId} onChange={choosePrimary} />
      )}
      {archivedContexts.length > 0 && (
        <details className="repository-context-archived">
          <summary>Archive済み ({archivedContexts.length})</summary>
          {archivedContexts.map((context) => (
            <ContextRow
              ...
              onToggle={() => {}}
              onSave={actions.saveContext}
              onRestore={() => actions.restoreContext(context)}
            />
          ))}
        </details>
      )}
      <div className="section-heading">
        <h3>新しいRepositoryを登録</h3>
      </div>
      <p className="field-help">
        AIセッションを自動で関連付けるにはLocal
        pathを登録してください。Themeを保存すると登録と紐付けが同時に完了します。
      </p>
          ...
          <select name="repository_new_provider" defaultValue="">
            ...
            {REPOSITORY_PROVIDERS.map((provider) => (
              <option key={provider} value={provider}>
                {REPOSITORY_PROVIDER_LABELS[provider] || provider}
              </option>
            ))}
          </select>
        </Field>
        <Field label="Label">
          <input name="repository_new_label" placeholder="例: Tasuken" />
        </Field>
      </div>
      <Field label="Remote URL">
        <input name="repository_new_url" type="text" placeholder="HTTPS / SSH / scp" />
      </Field>
      <Field label="Local path（AIセッション連携）">
        <input name="repository_new_local_path" placeholder="C:\\Users\\name\\projects\\example" />
      </Field>
      <div className="form-grid">
        <Field label="Default branch">
          <input name="repository_new_default_branch" placeholder="main" />
        </Field>
        <Field label="Subdirectory">
          <input name="repository_new_subdirectory" placeholder="apps/web" />
        </Field>
      </div>
    </section>
  );
...
export function TaskRepositoryContextFields({
  ...
  const contexts = ((data.repository_contexts || []) as unknown as RepositoryContext[]).filter(
    (context) => !context.deleted_at && context.active !== false,
  );
  const initialIds = Array.isArray(entity.repository_context_ids)
    ? entity.repository_context_ids.map(String)
  ...
  const modeLabel =
    REPOSITORY_CONTEXT_MODE_LABELS[mode as keyof typeof REPOSITORY_CONTEXT_MODE_LABELS] ||
    ...
    <details className="drawer-subsection repository-context-section drawer-disclosure">
      ...
      <div className="drawer-disclosure-body">
        <Field label="Taskへの関連付け">
          <select
            name="repository_context_mode"
            value={
              Object.prototype.hasOwnProperty.call(REPOSITORY_CONTEXT_MODE_LABELS, mode)
                ? mode
                : "inherit"
            }
            onChange={(event) => setMode(event.target.value)}
          >
            {Object.entries(REPOSITORY_CONTEXT_MODE_LABELS).map(([value, label]) => (
        ...
        {mode !== "inherit" && (
          <input type="hidden" name="repository_context_inputs_present" value="true" />
        )}
        {mode !== "inherit" && (
          <ContextOptions contexts={contexts} selectedIds={selectedIds} onToggle={toggle} />
        )}
        {mode !== "inherit" &&
          missingIds.map((id) => (
            <p className="field-help" key={id}>
        ...
        {mode !== "inherit" && selectedIds.length > 0 && (
          <PrimaryContextSelect contexts={contexts} value={primaryId} onChange={choosePrimary} />
        )}
        <div className="form-grid">
          <Field label="Subdirectory">
            <input
              ...
              defaultValue={String(entity.repository_subdirectory || "")}
              placeholder="apps/web"
            />
          </Field>
          <Field label="Branch hint">
            <input
              name="repository_branch_hint"
              defaultValue={String(entity.repository_branch_hint || "")}
