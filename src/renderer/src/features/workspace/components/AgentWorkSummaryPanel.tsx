              ...
              {group.rows.map((row) => {
                ...
                return (
                  <article className={`agent-work-row status-${session.status}`} key={session.id}>
                    ...
                    >
                      <span
                        className="agent-work-client-stack"
                        title={session.model_label || undefined}
                      >
                        <span className="agent-work-client">
                          <AI_ICON size={15} aria-hidden="true" />
                          {session.client_label ||
                            CLIENT_LABELS[session.client_kind] ||
                            session.client_kind}
                        </span>
                    ...
                    {expanded && (
                      <div className="agent-work-detail">
                        <div className="agent-handoff-line">
                          ...
                          <div>
                            <span>Outcome</span>
                            <p>{session.outcome?.summary || "作業中です。"}</p>
                          </div>
                        ...
                        <div className="agent-session-identity-detail">
                          <span>Session</span>
                          <code
                            title={`${session.client_kind}:${session.source_session_id || session.id}`}
                          >
                                ...
                                {(receipt.external_references || []).map((reference) => (
                                  ...
                                  >
                                    {reference.display_label ||
                                      EXTERNAL_REFERENCE_KIND_LABELS[reference.kind]}
