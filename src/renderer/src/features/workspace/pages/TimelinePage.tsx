...
export function TimelinePage({
  ...
  const updatePrefs = (patch: Partial<TimelinePrefs>) =>
  ...
  const timelineItems = persistedTimelineItems.map(
    (item) => optimisticTimelineItems[item.id] || item,
  );
  ...
  const groupKeys = rows
    .filter((row) => row.rowType === "theme")
    .map((row) => (row as Extract<typeof row, { rowType: "theme" }>).groupKey);
  ...
  useLayoutEffect(() => {
    ...
    const defaultScrollLeft = Math.max(
      0,
      (element.scrollWidth * todayLeft) / 100 - element.clientWidth / 2,
    );
  ...
  function rememberScrollPosition() {
    ...
    scrollSaveTimer.current = window.setTimeout(() => {
      ...
      const element = scrollRef.current;
      if (element) updatePrefs({ scrollLeft: Math.round(element.scrollLeft) });
    }, 250);
  }
  ...
  function resolveDropTarget(clientY: number): Item | null | undefined {
    ...
    const row = rows.find((entry) => {
      const height =
        entry.rowType === "item"
          ? ganttRowHeight(entry.laneItems.length ? entry.laneItems : [entry.item])
          : 44;
  ...
  async function renameItem(item: Item, title: string) {
    ...
    pushUndo({
      label: "名称変更",
      run: async () => {
        await saveEntities(timelineSaveItemOperations(item, v2));
      },
    });
  }
  ...
  async function moveItem(
    ...
    const dateChanged =
      currentSpan.start !== nextSpan.start ||
      currentSpan.end !== nextSpan.end ||
      item.due_date !== next.due_date;
    const parentChanged =
      item.parent_item_id !== next.parent_item_id ||
      timelineThemeId(item) !== timelineThemeId(next);
  ...
  function scheduleForPlanNode(planNodeId: string) {
    return v2.schedules.find(
      (schedule) => schedule.owner_type === "plan_node" && schedule.owner_id === planNodeId,
    );
  }
  ...
  function openPlanNode(item: Item) {
    ...
    openDrawer({
      type: "plan_node",
      mode: "edit",
      entity: { ...(planNode || item) } as Record<string, unknown>,
    });
  }
  ...
  async function createRangeFromRow(parent: Item, startDate: string, endDate: string) {
    ...
    const siblings = timelineItems.filter(
      (item) =>
        item.parent_item_id === parent.id && timelineThemeId(item) === timelineThemeId(parent),
    );
      ...
      [
        {
          action: "save",
          type: "plan_node",
          entity: planNode as unknown as SaveOperation["entity"],
        },
        {
          action: "save",
          type: "schedule",
          entity: schedule as unknown as SaveOperation["entity"],
        },
      ],
    ...
    pushUndo({
      label: "計画追加",
      run: async () => {
        await removeEntityQuiet("plan_node", planNodeId);
      },
    });
    ...
    openDrawer({
      type: "plan_node",
      mode: "edit",
      entity: { ...planNode, _schedule: schedule, _focusTitle: true } as unknown as Record<
        string,
        unknown
      >,
    });
  ...
  async function reorderItemToTarget(item: Item, target: Item) {
    ...
    if (
      ...
      timelineThemeId(item) !== timelineThemeId(target)
    ) {
    ...
    const siblings = timelineItems
      .filter(
        (entry) =>
          ...
          entry.parent_item_id === item.parent_item_id &&
          timelineThemeId(entry) === timelineThemeId(item),
      )
      .sort(
        (a, b) =>
    ...
    const ops = reordered.flatMap((entry, orderIndex) =>
      timelineSaveItemOperations({ ...entry, sort_order: (orderIndex + 1) * 10 }, v2),
    );
    ...
    setOptimisticTimelineItems((current) => ({
      ...
      ...Object.fromEntries(
        reordered.map((entry, orderIndex) => [
          entry.id,
          { ...entry, sort_order: (orderIndex + 1) * 10 },
        ]),
      ),
    }));
    try {
  ...
  async function deleteItem(item: Item) {
    ...
    const deps = v2.plan_dependencies.filter(
      (dep) => dep.plan_node_id === planNode.id || dep.depends_on_plan_node_id === planNode.id,
    );
    ...
    pushUndo({
      ...
      run: async () => {
        await saveEntities([
          ...
          ...deps.map((dep) => ({
            action: "save" as const,
            type: "plan_dependency" as const,
            entity: dep as unknown as SaveOperation["entity"],
          })),
        ]);
      },
    });
  ...
  return (
    <div className="page timeline-wide">
      <PageHeader route="timeline">
        <Button variant="secondary" onClick={() => setSlideTimelineOpen(true)}>
        ...
        <Button
          ...
          onClick={() =>
            openDrawer({
              type: "plan_node",
              mode: "edit",
              entity: { node_type: "phase", node_state: "planned" },
            })
          }
        >
          <IconPlus size={16} />
      ...
      <section className="timeline-toolbar toolbar-row panel">
        ...
        <div className="segmented" aria-label="表示範囲">
          {RANGE_BUFFER_OPTIONS.map((option) => (
            <button
              key={option.value}
              className={rangeBufferMonths === option.value ? "is-active" : ""}
              onClick={() => updatePrefs({ rangeBufferMonths: option.value })}
            >
              {option.label}
            </button>
          ))}
        ...
        <div className="segmented" aria-label="表示倍率">
          {LONG_RANGE_ZOOM_PRESETS.map(({ id, label, dayWidth: pw }) => (
            <button
              key={id}
              className={Math.abs(dayWidth - pw) < 0.5 ? "is-active" : ""}
              onClick={() => applyZoom(pw)}
            >
              {label}
            </button>
          ))}
        ...
        <ToolbarOverflow label="表示" ariaLabel="タイムラインの表示切替">
          <label className="toggle">
            <input
              ...
              onChange={(event) => updatePrefs({ showCompleted: event.target.checked })}
            />
            完了タスク
          </label>
          <label className="toggle">
            <input
              ...
              onChange={(event) => updatePrefs({ showDependencies: event.target.checked })}
            />
            依存線
          </label>
          <label className="toggle">
            <input
              ...
              onChange={(event) => updatePrefs({ showLightning: event.target.checked })}
            />
            イナズマ線
          </label>
          <hr />
          {SHORT_RANGE_ZOOM_PRESETS.map(({ id, label, dayWidth: pw }) => (
            <button key={id} type="button" role="menuitem" onClick={() => applyZoom(pw)}>
              {label}表示にする
            </button>
          ))}
        </ToolbarOverflow>
      ...
      <section className={`split-gantt panel ${connecting ? "is-connecting" : ""}`}>
        {connecting && (
          <div className="connect-status-popover" role="status" aria-live="polite">
            <span>
              {connecting.sourceTitle ? (
                <>
                  先行: <strong>{connecting.sourceTitle}</strong> → 後続タスクをクリック
                </>
              ) : (
                "先行タスクをクリック"
              )}
            ...
            if (row.rowType === "theme") {
              ...
              return (
                <div className="gantt-theme-row" key={`theme-${row.groupKey}`}>
                  <button
                    className="gantt-theme-toggle"
                    onClick={() =>
                      setCollapsedThemes((current) =>
                        current.includes(row.groupKey)
                          ? current.filter((key) => key !== row.groupKey)
                  ...
                  <span className="gantt-theme-count">
                    実施事項 {row.initiativeCount} / 計画 {row.planCount}
                  </span>
                </div>
              );
            }
              ...
              >
                ...
                >
                  {editingTitle?.id === item.id ? (
                    <input
                      ...
                      onChange={(event) =>
                        setEditingTitle({ id: item.id, value: event.target.value })
                      }
                    ...
                    <>
                      <button
                        ...
                        title={item.title}
                        onClick={(e) => {
                          if ((e.ctrlKey || e.metaKey) && !isPlan) {
                            handleCtrlClick(item);
                            return;
                          }
                          isPlan && !connectMode && !connecting
                            ? setEditingTitle({ id: item.id, value: item.title })
                            : connecting || connectMode
                              ? startConnecting(item)
                              : openPlanNode(item);
                        }}
                      >
                        {item.title}
                ...
                {isPlan ? (
                  <div className="gantt-row-actions">
                    {dropSortTargetId === item.id && draggingSortId && (
                      <span className="gantt-drop-label">ここへ移動</span>
                    )}
                    <button
                    ...
                    <button
                      className="gantt-row-action danger"
                  ...
                  <StatusBadge
                    value={timelineItemStatusValue(item)}
                    label={timelineItemStatusLabel(item)}
                  />
                )}
              </div>
            );
        ...
        <div className="gantt-scroll" ref={scrollRef} onScroll={rememberScrollPosition}>
          <div
            className="gantt-canvas"
            ref={canvasRef}
            style={
              { width: canvasWidth, "--gantt-grid-image": gridBackground } as React.CSSProperties
            }
            onClick={() => selectedDep && setSelectedDep(null)}
          >
            ...
            {rows.map((row) => {
              if (row.rowType === "theme")
                return <div className="gantt-canvas-theme-row" key={`theme-${row.groupKey}`} />;
              if (row.rowType === "milestones") {
                const colorKey = themeColor(row.theme, themes.indexOf(row.theme ?? themes[0]));
                return (
                  <MilestoneLane
                    key={`milestones-${row.groupKey}`}
                    ...
                    onMove={(item, delta) => {
                      selectTimelineItem(item);
                      moveItem(item, delta, "move");
                    }}
                    onCreateMilestone={(date) => createMilestone(row.theme?.id || null, date)}
              ...
              return (
                <GanttItemRow
                  key={row.item.id}
                  item={row.item}
                  laneItems={row.laneItems}
                  range={range}
                  hint={dateHint}
                  selectedItemId={selectedTimelineItemId}
                  onOpen={(item) => (connecting ? startConnecting(item) : openPlanNode(item))}
                  onSelect={selectTimelineItem}
                  onMove={(item, delta, mode, targetParent) => {
                    selectTimelineItem(item);
                    moveItem(item, delta, mode, targetParent);
                  }}
                  connecting={connecting}
                  onConnect={startConnecting}
                  onCreateRange={createRangeFromRow}
                  themeColorKey={colorKey}
      ...
      {outsideCount > 0 && (
        <div className="timeline-range-note">
          表示範囲外の計画が {outsideCount} 件あります。前後期間を広げると確認できます。
        </div>
      )}
      {slideTimelineOpen && (
        <SlideTimelineDialog
