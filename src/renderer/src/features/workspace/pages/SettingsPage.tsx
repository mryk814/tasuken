...
export function SettingsPage({
  ...
  const [aiConfigState, setAiConfigState] = useState<"loading" | "empty" | "error" | "success">(
    "loading",
  );
  ...
  const [aiModelCapabilities, setAiModelCapabilities] = useState<AiCapability[]>([
    "text",
    "streaming",
    "tool_calling",
    "structured_output",
  ...
  useEffect(() => {
    ...
    workspaceApi
      ...
      .then((config) => {
        ...
        const provider =
          config.providers.find((candidate) => candidate.id === config.defaultProviderProfileId) ||
          config.providers[0];
        const model =
          config.models.find((candidate) => candidate.id === config.defaultModelProfileId) ||
          (provider
  ...
  useEffect(() => {
    ...
    workspaceApi
      .getAiFeatureAvailability("note_assistant", aiProviderId, aiModelProfileId)
  ...
  async function saveAiSettings(clearApiKey = false) {
    ...
    try {
      ...
      const provider =
        providerConfig.providers.find((candidate) => candidate.id === aiProviderId) ||
      ...
      const existingModel = providerConfig.models.find(
        (candidate) =>
          candidate.id === aiModelProfileId && candidate.providerProfileId === provider.id,
      );
      ...
      const savedModel =
        config.models.find(
          (candidate) =>
            ...
            (candidate.id === existingModel?.id || candidate.model === aiModel),
        ) || config.models.find((candidate) => candidate.providerProfileId === provider.id);
      setAiConfig(config);
      setAiConfigState(config.providers.length === 0 ? "empty" : "success");
      setAiProviderId(provider.id);
      ...
      setAiApiKey("");
      setToast(
        clearApiKey ? "credentialを削除しました。" : "AI profileを安全に保存しました。",
        "success",
      );
    } catch (error) {
      setToast(
        `AI設定を保存できませんでした。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    } finally {
      setAiBusy(false);
  ...
  function selectAiProvider(id: string, sourceConfig: AiProviderConfig | null = aiConfig) {
    ...
    if (!provider) return;
    const model =
      sourceConfig.models.find(
        (candidate) =>
          candidate.id === sourceConfig.defaultModelProfileId &&
          candidate.providerProfileId === provider.id,
  ...
  async function testAiProviderConnection() {
    ...
    setAiTestMessage("");
    try {
      const result = await workspaceApi.testAiConnection({
        providerProfileId: aiProviderId,
        modelProfileId: aiModelProfileId || undefined,
      });
  ...
  async function setAiProviderDefault() {
    ...
    try {
      ...
      setToast(
        `default providerを変更できませんでした。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    }
  }
  ...
  async function setAiModelDefault() {
    ...
    try {
      ...
      setToast(
        `default modelを変更できませんでした。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    }
  }
  ...
  async function deleteAiProvider() {
    ...
    try {
      ...
      setToast(
        `provider profileを削除できませんでした。${error instanceof Error ? error.message : String(error)}`,
        "danger",
      );
    }
  }
      ...
      : aiConfigState === "error"
        ...
        : aiConfigState === "empty" || !aiConfig || aiConfig.providers.length === 0
          ...
          : aiConfig.providers.some((provider) => provider.enabled && provider.credentialConfigured)
            ? {
                label: `${aiConfig.providers.length} profile · ${aiConfig.models.length} model`,
      ...
      : aiTestStatus === "connected"
        ...
        : aiTestStatus === "idle"
          ...
          : aiTestStatus === "missing_credential" || aiTestStatus === "model_unavailable"
            ? {
                label:
                  aiTestStatus === "missing_credential" ? "credential未設定" : "model unavailable",
      ...
      <div className="settings-layout">
        ...
        <div className="settings-category-content">
          ...
          <div className="settings-grid">
            ...
            <section
              className="panel settings-form ai-provider-settings-panel"
              hidden={activeSection !== "ai-mcp"}
            >
              <div className="settings-section-heading">
                <h2>AI Provider</h2>
              ...
              <p className="field-help">
                Providerとmodelを分けて管理します。返答はPending
                Proposalへ入り、別providerへの自動fallbackはありません。
              </p>
              {aiConfigState === "loading" && (
                <p className="field-help">AI profileを読み込んでいます…</p>
              )}
              {aiConfigState === "error" && (
                <div className="settings-current-state">
                  ...
                  <Button
                    variant="secondary"
                    compact
                    onClick={() => setAiConfigReloadToken((value) => value + 1)}
                  >
                    再試行
                  </Button>
                </div>
              ...
              {aiConfigState === "empty" && (
                <p className="field-help">
                  Provider profileがまだありません。追加してから接続をテストしてください。
                </p>
              )}
              <div className="ai-profile-list" aria-label="Provider profiles">
                {(aiConfig?.providers || []).map((provider) => (
                  <button
                    type="button"
                    key={provider.id}
                    className={`ai-profile-row ${provider.id === aiProviderId ? "is-selected" : ""}`}
                  ...
                  >
                    <span>
                      ...
                      <small>
                        ...
                        {provider.endpointExposure === "local_private"
                          ? "local/private"
                          : "external"}{" "}
                        ·{" "}
                        {provider.credentialConfigured ? "credential設定済み" : "credential未設定"}
                      </small>
                    </span>
                    <span>
                      {provider.id === aiConfig?.defaultProviderProfileId
                        ? "default"
                        : provider.enabled
                          ? "enabled"
                          : "disabled"}
                ...
                <Button variant="secondary" compact onClick={startNewAiProvider}>
                  Providerを追加
                </Button>
              </div>
                ...
                <div className="settings-detail-body">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => setAiProviderLabel(event.target.value)}
                      placeholder="OpenAI"
                    />
                  </label>
                  <label>
                    ...
                    <select
                      value={aiAdapterKind}
                      onChange={(event) => changeAiAdapter(event.target.value as AiAdapterKind)}
                    >
                  ...
                  <label>
                    ...
                    <select
                      value={aiAuthKind}
                      onChange={(event) => setAiAuthKind(event.target.value as AiAuthKind)}
                    >
                      {AI_AUTHS_BY_ADAPTER[aiAdapterKind].map((authKind) => (
                        <option value={authKind} key={authKind}>
                          {authKind === "api_key"
                            ? "API key"
                  ...
                  <label>
                    ...
                    <select
                      value={aiApiSurface}
                      onChange={(event) => setAiApiSurface(event.target.value as AiApiSurface)}
                    >
                      {AI_SURFACES_BY_ADAPTER[aiAdapterKind].map((apiSurface) => (
                        <option value={apiSurface} key={apiSurface}>
                          {apiSurface === "responses"
                            ? "Responses"
                            : apiSurface === "chat_completions"
                              ? "Chat Completions"
                              : "Native provider API（未実装）"}
                        </option>
                      ))}
                    </select>
                  </label>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      placeholder={
                        aiAdapterKind === "openai-native"
                          ? "固定: https://api.openai.com"
                          : "https://…（credentialなし）"
                      }
                    />
                  </label>
                  {selectedAiProvider && (
                    <p className="field-help endpoint-exposure-note">
                      Endpoint scope:{" "}
                      <strong>
                        {selectedAiProvider.endpointExposure === "local_private"
                          ? "local/private"
                          : "external"}
                      </strong>
                      {selectedAiProvider.endpointExposure === "local_private"
                  ...
                  <div className="settings-meta-list ai-profile-meta-fields">
                    <label>
                      Organization
                      <input
                        value={aiOrganization}
                        onChange={(event) => setAiOrganization(event.target.value)}
                      />
                    </label>
                    <label>
                      Project
                    ...
                    <label>
                      ...
                      <input
                        value={aiRegion}
                        onChange={(event) => setAiRegion(event.target.value)}
                      />
                    </label>
                    <label>
                      Deployment
                      <input
                        value={aiDeployment}
                        onChange={(event) => setAiDeployment(event.target.value)}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        min="30"
                        max="600"
                        value={Math.round(aiRequestTimeoutMs / 1000)}
                        onChange={(event) =>
                          setAiRequestTimeoutMs(Number(event.target.value) * 1000)
                        }
                      />
                    </label>
                  </div>
                  ...
                  <div className="settings-action-row">
                    ...
                    <Button
                      variant="secondary"
                      disabled={aiBusy || !aiProviderId}
                      onClick={testAiProviderConnection}
                    >
                    ...
                    <Button
                      ...
                      disabled={
                        aiBusy ||
                        !aiProviderId ||
                        aiProviderId === aiConfig?.defaultProviderProfileId
                      }
                      onClick={setAiProviderDefault}
                    >
                      default providerにする
                    </Button>
                  </div>
                  <IntegrationStatus
                    label={aiTestSummary.label}
                    tone={aiTestSummary.tone}
                    detail={aiTestMessage}
                  />
                  {aiTestMessage && (
                    <p
                      className={`field-help ${aiTestStatus === "connection_failed" ? "form-error" : ""}`}
                    >
                      {aiTestMessage}
                    </p>
                  )}
              ...
              <details className="settings-detail" open>
                ...
                <div className="settings-detail-body">
                  <div className="ai-model-list">
                    {(aiConfig?.models || [])
                      .filter((model) => model.providerProfileId === aiProviderId)
                      .map((model) => (
                        <button
                          type="button"
                          key={model.id}
                          className={`ai-profile-row ${model.id === aiModelProfileId ? "is-selected" : ""}`}
                          onClick={() => {
                            setAiModelProfileId(model.id);
                            setAiModel(model.model);
                            setAiModelDisplayName(model.displayName);
                        ...
                        >
                          <span>
                            ...
                            <small className="mono-value">
                              {model.model} · {model.capabilities.join(", ")}
                            </small>
                          </span>
                          <span>
                            {model.id === aiConfig?.defaultModelProfileId
                              ? "default"
                              : model.lifecycle}
                          </span>
                        </button>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => setAiModelDisplayName(event.target.value)}
                      placeholder={aiModel || "表示名"}
                    />
                  </label>
                  <label>
                    Lifecycle
                    <select
                      value={aiModelLifecycle}
                      onChange={(event) =>
                        setAiModelLifecycle(event.target.value as AiModelLifecycle)
                      }
                    >
                  ...
                  <fieldset className="ai-capability-list">
                    ...
                    {(
                      ...
                      <label key={capability}>
                        <input
                          type="checkbox"
                          checked={aiModelCapabilities.includes(capability)}
                          onChange={(event) =>
                            setAiModelCapabilities((current) =>
                  ...
                  <div className="settings-action-row">
                    <Button
                      variant="primary"
                      disabled={aiBusy || !aiModel.trim() || !aiProviderId}
                      onClick={() => saveAiSettings(false)}
                    >
                      Modelを保存
                    </Button>
                    <Button
                      variant="secondary"
                      disabled={
                        !aiModelProfileId || aiConfig?.defaultModelProfileId === aiModelProfileId
                      }
                      onClick={setAiModelDefault}
                    >
                      default modelにする
                    </Button>
                  </div>
                  {aiAvailability && (
                    <p className={`field-help ${aiAvailability.available ? "" : "form-error"}`}>
                      {aiAvailability.available
                        ? "Note AI: 利用可能"
                        : `Note AI: ${aiAvailability.reason === "capability_missing" ? `capability不足（${aiAvailability.missing.join(", ")}）` : aiAvailability.reason === "model_unavailable" ? "model unavailable" : "利用できません"}`}
                    </p>
                  )}
                </div>
              </details>
              {selectedAiProvider && (
                <div className="settings-danger-zone">
                  ...
                  <Button variant="ghost" disabled={aiBusy} onClick={deleteAiProvider}>
                    このprovider profileを削除
                  </Button>
                </div>
              )}
            </section>
