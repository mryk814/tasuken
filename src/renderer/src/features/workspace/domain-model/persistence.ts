...
function activitySessionId(entityType: string, entityId: string, context: SaveContext): string {
  ...
  if (
    context.newSession ||
    !previous ||
    at - previous.lastAt > ACTIVITY_SESSION_IDLE_MS ||
    at < previous.lastAt
  ) {
...
export function buildChangeEventOperation(
  ...
  const occurredAt =
    changeType === "completed" && after && typeof after === "object" && "completed_at" in after
...
export function buildSaveTaskOperations(task: Task, context: SaveContext = {}): SaveOperation[] {
  return [
    saveOperation("task", task as unknown as Entity, {
      source: context.source || "manual",
    ...
    buildChangeEventOperation(
      "task",
      task.id,
      task.completed_at ? "completed" : "updated",
      context,
      undefined,
      task,
    ),
...
export function buildSaveWaitingOperations(
  waiting: Waiting,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("waiting", waiting as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
    buildChangeEventOperation(
      "waiting",
      waiting.id,
      waiting.state === "received" ? "completed" : "updated",
      context,
      undefined,
      waiting,
    ),
...
export function buildSavePlanNodeOperations(
  planNode: PlanNode,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("plan_node", planNode as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
    buildChangeEventOperation(
      "plan_node",
      planNode.id,
      planNode.state === "done" ? "completed" : "updated",
      context,
      undefined,
      planNode,
    ),
...
export function buildSaveScheduleOperations(
  schedule: Schedule,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("schedule", schedule as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
    buildChangeEventOperation(
...
export function buildSaveCaptureEntryOperations(
  entry: CaptureEntry,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("capture_entry", entry as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
...
export function buildSaveResourceOperations(
  resource: Resource,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("resource", resource as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
...
export function buildSaveNoteOperations(note: Note, context: SaveContext = {}): SaveOperation[] {
  return [
    saveOperation("note", note as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
...
export function buildSaveFeedPostOperations(
  post: FeedOwnPost,
  context: SaveContext = {},
): SaveOperation[] {
  return [
    saveOperation("feed_post", post as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
    buildChangeEventOperation("feed_post", post.id, "updated", context, undefined, post),
  ];
}
...
export function buildTriageCaptureEntryOperations(
  ...
  return [
    saveOperation("capture_entry", next as unknown as Entity, {
      source: context.source || "manual",
      reason: context.reason || undefined,
    }),
...
export async function saveWaiting(
  saveEntities: (operations: SaveOperation[], message?: string) => Promise<Entity[]>,
  waiting: Waiting,
  context?: SaveContext,
): Promise<Entity[]> {
...
export async function savePlanNode(
  saveEntities: (operations: SaveOperation[], message?: string) => Promise<Entity[]>,
  planNode: PlanNode,
  context?: SaveContext,
): Promise<Entity[]> {
...
export async function saveCaptureEntry(
  saveEntities: (operations: SaveOperation[], message?: string) => Promise<Entity[]>,
  entry: CaptureEntry,
  context?: SaveContext,
): Promise<Entity[]> {
  return saveEntities(
    buildSaveCaptureEntryOperations(entry, context),
    "CaptureEntryを保存しました。",
  );
}
...
export async function triageCaptureEntry(
  ...
  return saveEntities(
    buildTriageCaptureEntryOperations(entry, target, context),
    "CaptureEntryを整理しました。",
  );
}
