...
export function usePreference<K extends PreferenceId>(
  id: K,
  scopeKey = "",
): [PreferenceValueMap[K], (next: PreferenceValueMap[K] | ((current: PreferenceValueMap[K]) => PreferenceValueMap[K])) => void] {
  ...
  const latestWriteRef = useRef<{ effectGeneration: number; sequence: number; externalGeneration: number } | null>(null);
  ...
  useEffect(() => {
    ...
    void workspaceApi.getViewPreferences().then((envelope) => {
      ...
      valueRef.current = next;
      setValue(next);
      void workspaceApi.setViewPreference(id, normalizedScopeKey, next, definition.schemaVersion).then((change) => {
        revisionRef.current = change.revision;
        removeLegacy(migrated.keys);
      }).catch((error) => {
        window.dispatchEvent(new CustomEvent("tasken:preference-save-error", { detail: { id, error } }));
      });
    }).catch((error) => {
      window.dispatchEvent(new CustomEvent("tasken:preference-save-error", { detail: { id, error } }));
    });
    const unsubscribe = workspaceApi.onViewPreferenceChanged((change) => {
      if (!active || change.id !== id || change.scopeKey !== normalizedScopeKey || change.revision < revisionRef.current) return;
  ...
  const setPreference = useCallback((next: PreferenceValueMap[K] | ((current: PreferenceValueMap[K]) => PreferenceValueMap[K])) => {
    ...
    setValue(normalized);
    void workspaceApi.setViewPreference(id, normalizedScopeKey, normalized, definition.schemaVersion).then((change) => {
      revisionRef.current = Math.max(revisionRef.current, change.revision);
    }).catch((error) => {
      if (shouldRollbackPreferenceWrite({
        effectGeneration,
        currentEffectGeneration: effectGenerationRef.current,
        writeSequence,
