...
function readJson(fileSystem, filePath, maxBytes = 256 * 1024) {
  if (isSymbolicLink(fileSystem, filePath))
    throw new Error("manifestにsymlink/junctionは利用できません。");
  const stat = fileSystem.statSync(filePath);
  if (!stat.isFile() || stat.size > maxBytes)
    throw new Error("manifestの形式またはサイズが不正です。");
  return JSON.parse(fileSystem.readFileSync(filePath, "utf8"));
}

function writeJson(fileSystem, filePath, value, operationId) {
  return writeAtomicTextFile(
    filePath,
...
function assertPlan(plan) {
  ...
  if (
    ...
    names.some((name, index) => name !== EXPECTED_FILE_NAMES[index])
  ) {
  ...
  if (
    plan.manifest?.themeId !== plan.theme_id ||
    plan.manifest?.contentHash !== plan.content_hash
  ) {
...
function validatePublishedPack(fileSystem, directory, plan) {
  ...
  if (
    manifest.schema !== THEME_AI_PACK_SCHEMA ||
    manifest.themeId !== plan.theme_id ||
    manifest.contentHash !== plan.content_hash
  )
    return false;
  if (!Array.isArray(manifest.files) || manifest.files.length !== EXPECTED_FILE_NAMES.length)
    return false;
  for (const file of plan.files) {
    ...
    try {
      if (markdownSignature(fileSystem.readFileSync(filePath, "utf8")) !== file.content_hash)
        return false;
    } catch {
      return false;
    }
...
function validatePackAgainstOwnManifest(fileSystem, directory, themeId, operationId, phase) {
  ...
  if (
    manifest.files.some(
      (entry, index) =>
        entry?.name !== EXPECTED_FILE_NAMES[index] || typeof entry?.contentHash !== "string",
    )
  )
    return false;
  for (const described of manifest.files) {
    ...
    if (!isWithin(directory, filePath) || isSymbolicLink(fileSystem, filePath)) return false;
    try {
      if (markdownSignature(fileSystem.readFileSync(filePath, "utf8")) !== described.contentHash)
        return false;
    } catch {
      return false;
    }
  }
  const contentHash = markdownSignature(
    JSON.stringify(
...
function snapshotExistingPackManifest(fileSystem, directory, manifest, themeId, operationId) {
  ...
  const files = EXPECTED_FILE_NAMES.map((name) => {
    ...
    return {
      ...
      includedEntityIds: Array.isArray(described?.includedEntityIds)
        ? described.includedEntityIds
        : [],
    };
  });
  return {
    ...manifest,
    contentHash: markdownSignature(
      JSON.stringify(
        files.map((file) => ({
          name: file.name,
          content_hash: file.contentHash,
  ...
  if (override) {
      ...
      try {
        if (!themeFolderManifestMatches(readJson(fileSystem, markerPath, 32 * 1024), id)) {
          return {
            ...
            dirty: true,
            retryPending: false,
            reason: "theme_manifest_mismatch",
          };
        }
      } catch {
        return {
    ...
    if (isDirectory(fileSystem, themesDirectory) && !isSymbolicLink(fileSystem, themesDirectory)) {
      for (const entry of fileSystem.readdirSync(themesDirectory, { withFileTypes: true })) {
        ...
        if (!isWithin(themesDirectory, candidate) || isSymbolicLink(fileSystem, candidate))
          continue;
        ...
        try {
          ...
          if (themeFolderManifestMatches(marker, id)) matches.push(candidate);
            ...
            entry.name === preferredSegment &&
            marker?.schema !== THEME_FOLDER_MANIFEST_SCHEMA
          ) {
            return {
              status: "identity_conflict",
              dirty: true,
    ...
    if (matches.length === 1) {
      ...
      if (!isWithin(themesDirectory, themeFolder))
        throw new Error("Theme folderが同期rootの外にあります。");
      const markerPath = path.join(themeFolder, THEME_FOLDER_MANIFEST);
      if (fileSystem.existsSync(themeFolder)) {
        ...
        if (!fileSystem.existsSync(markerPath)) {
          return {
            ...
            dirty: true,
            retryPending: false,
            reason: "unclaimed_preferred_folder",
          };
        }
        try {
          if (!themeFolderManifestMatches(readJson(fileSystem, markerPath, 32 * 1024), id)) {
...
export function ensureThemeAiPackLocation(
  location,
  { fileSystem = fs, operationId = randomUUID() } = {},
) {
...
export function publishThemeAiPack({
  ...
  if (path.basename(targetDirectory) !== THEME_AI_PACK_DIRECTORY)
    throw new Error("AI Pack directory名が不正です。");
  ...
  if (current.state === "current")
    return { ...current, state: "skipped", retryPending: false, written: false };

  const stageDirectory = path.join(
    parentDirectory,
    `.${THEME_AI_PACK_DIRECTORY}.${operationId}.staging`,
  );
  ...
  if (fileSystem.existsSync(stageDirectory) || fileSystem.existsSync(backupDirectory)) {
    throw new Error(
      "同じTheme AI Pack operationの残骸があります。復旧してから再試行してください。",
    );
  }
  ...
  try {
    ...
    for (const file of normalized.files) {
      writeAtomicTextFile(
        path.join(stageDirectory, file.name),
        file.content,
        `${operationId}-${EXPECTED_FILE_NAMES.indexOf(file.name)}`,
        fileSystem,
      );
    }
    writeJson(
      fileSystem,
      path.join(stageDirectory, THEME_AI_PACK_MANIFEST),
      operationManifest(normalized, operationId, "staged"),
      `${operationId}-manifest`,
    );
    fsyncDirectory(fileSystem, stageDirectory);
    if (!validatePublishedPack(fileSystem, stageDirectory, normalized))
      throw new Error("stagingしたAI Packのhashを検証できませんでした。");
    receipt = { ...receipt, phase: "staged" };
    writeReceipt(fileSystem, recoveryRoot, receipt);

    if (fileSystem.existsSync(targetDirectory)) {
      if (isSymbolicLink(fileSystem, targetDirectory))
        throw new Error("既存AI Packにsymlink/junctionは利用できません。");
      const previous = readPackManifest(fileSystem, targetDirectory);
      previousManifest = previous;
      if (
        previous.schema !== THEME_AI_PACK_SCHEMA ||
        text(previous.themeId) !== normalized.theme_id ||
        isSymbolicLink(fileSystem, targetDirectory)
      ) {
        throw new Error("既存AI PackのTheme identityを検証できませんでした。");
      }
      writeJson(
    ...
    writeJson(
      fileSystem,
      path.join(targetDirectory, THEME_AI_PACK_MANIFEST),
      operationManifest(normalized, operationId, "published"),
    ...
    if (backupCreated) {
      try {
        removeValidatedOperationDirectory(
          fileSystem,
          backupDirectory,
          normalized.theme_id,
          operationId,
    ...
    try {
      if (published && fileSystem.existsSync(targetDirectory)) {
        failedDirectory = path.join(
          parentDirectory,
          `.${THEME_AI_PACK_DIRECTORY}.${operationId}.failed`,
        );
        fileSystem.renameSync(targetDirectory, failedDirectory);
      }
      if (
        backupCreated &&
        fileSystem.existsSync(backupDirectory) &&
        !fileSystem.existsSync(targetDirectory)
      ) {
        ...
        writeJson(
          ...
          {
            ...backupManifest,
            operation: previousManifest?.operation || {
              ...backupManifest.operation,
              phase: "published",
            },
          },
    ...
    if (!recoveryRequired) {
      try {
        if (fileSystem.existsSync(stageDirectory)) {
          if (
            stageCreated &&
            !fileSystem.existsSync(path.join(stageDirectory, THEME_AI_PACK_MANIFEST))
          ) {
        ...
        if (failedDirectory && fileSystem.existsSync(failedDirectory)) {
          ...
          if (
            !["staged", "published"].includes(phase) ||
            !validatePackAgainstOwnManifest(
              fileSystem,
              failedDirectory,
              normalized.theme_id,
              operationId,
    ...
    if (recoveryRequired) {
      writeReceipt(fileSystem, recoveryRoot, {
        ...receipt,
...
function receiptPathsAreSafe(receipt) {
  ...
  return Boolean(
    ...
    path.resolve(receipt.stageDirectory) ===
      path.join(parent, `.${THEME_AI_PACK_DIRECTORY}.${operationId}.staging`) &&
    path.resolve(receipt.backupDirectory) ===
      path.join(parent, `.${THEME_AI_PACK_DIRECTORY}.${operationId}.backup`),
  );
}
...
export function recoverThemeAiPackOperations({ recoveryDirectory, fileSystem = fs } = {}) {
  ...
  for (const name of fileSystem
    ...
    try {
      ...
      if (
        [target, stage, backup].some(
          (candidate) => fileSystem.existsSync(candidate) && isSymbolicLink(fileSystem, candidate),
        )
      ) {
        throw new Error("recovery対象にsymlink/junctionは利用できません。");
      }
      const targetManifest = fileSystem.existsSync(target)
        ? readPackManifest(fileSystem, target)
        : null;
      const stageManifest = fileSystem.existsSync(stage)
        ? readPackManifest(fileSystem, stage)
        : null;
      const backupManifest = fileSystem.existsSync(backup)
        ? readPackManifest(fileSystem, backup)
        : null;

      if (
        targetManifest &&
        validatePackAgainstOwnManifest(fileSystem, target, themeId, operationId, "published")
      ) {
        if (stageManifest)
          removeValidatedOperationDirectory(fileSystem, stage, themeId, operationId, "staged");
        if (backupManifest)
          removeValidatedOperationDirectory(fileSystem, backup, themeId, operationId, "backup");
      ...
      if (
        targetManifest &&
        !backupManifest &&
        validatePackAgainstOwnManifest(fileSystem, target, themeId, operationId, "backup")
      ) {
        writeJson(
          fileSystem,
          path.join(target, THEME_AI_PACK_MANIFEST),
          {
            ...targetManifest,
      ...
      if (
        targetManifest &&
        validatePackAgainstOwnManifest(fileSystem, target, themeId, operationId, "staged")
      ) {
        writeJson(
          fileSystem,
          path.join(target, THEME_AI_PACK_MANIFEST),
      ...
      if (
        !targetManifest &&
        backupManifest &&
        validatePackAgainstOwnManifest(fileSystem, backup, themeId, operationId, "backup")
      ) {
      ...
      if (
        ...
        !backupManifest &&
        stageManifest &&
        validatePackAgainstOwnManifest(fileSystem, stage, themeId, operationId, "staged")
      ) {
      ...
      results.push({
        operationId: text(receipt?.operationId) || name.replace(/\.json$/, ""),
        state: "recovery_required",
        error: errorText(error),
      });
    }
