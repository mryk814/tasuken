...
const proposalPayloadTypes = new Set([
  "items",
  "notes",
  "links",
  "knowledge_nodes",
...
const artifactGeneratedByValues = new Set([
  "chatgpt",
  "claude",
  "copilot",
  "gemini",
...
const artifactLinkTypes = new Set([
  "url",
  "local_path",
  "shared_path",
  "onedrive",
...
const mediaAvailabilities = new Set([
  "available",
  "missing",
  "changed",
  "unsafe_source",
...
function validateWorkItemList(value, field, maxItems = 100) {
  ...
  if (!Array.isArray(value) || value.length > maxItems)
    throw new Error(`${field}は${maxItems}件以内の配列にしてください。`);
  for (const item of value) {
    if (typeof item !== "string" || !item.trim())
      throw new Error(`${field}は空でない文字列の配列にしてください。`);
...
function validateTaskRepeatRule(rule) {
  ...
  if (!taskRepeatFrequencies.has(rule.frequency))
    throw new Error("task.repeat_rule.frequencyが不正です。");
  const interval = Number(rule.interval);
  if (!Number.isInteger(interval) || interval < 1 || interval > 365) {
    throw new Error("task.repeat_rule.intervalは1以上365以下で指定してください。");
  }
  if (!taskRepeatNextFromValues.has(rule.next_from))
    throw new Error("task.repeat_rule.next_fromが不正です。");
...
function validateTaskChecklist(items) {
  ...
  for (const item of items) {
    ...
    if (typeof item.id !== "string" || !item.id.trim())
      throw new Error("task.checklist_items.idを入力してください。");
    if (typeof item.title !== "string" || !item.title.trim())
      throw new Error("task.checklist_items.titleを入力してください。");
    if (item.title.length > 200)
      throw new Error("task.checklist_items.titleは200文字以内で入力してください。");
    if (typeof item.done !== "boolean") throw new Error("task.checklist_items.doneが不正です。");
    if (!Number.isFinite(Number(item.sort_order)))
      throw new Error("task.checklist_items.sort_orderが不正です。");
    if (
...
function validateSketchDocument(document) {
  ...
  if (document.pages.length > 200)
    throw new Error("sketch.document.pagesは200ページ以内にしてください。");
...
export function assertItemParentAcyclic(
  items,
  entity,
  message = "親Itemに自分自身または子孫を指定すると循環します。別の親Itemを選んでください。",
) {
  if (!entity.parent_item_id) return;
  const entityId = String(entity.id);
  const byId = new Map(
    items.filter((item) => !item.deleted_at).map((item) => [String(item.id), item]),
  );
...
export function validateEntity(type, input) {
  ...
  if (type === "theme" || type === "project" || type === "task")
    normalizeRepositoryLinkFields(type, input);
  ...
  for (const field of urlFields) {
    if (input[field] != null && input[field] !== "" && !isAllowedExternalUrl(input[field])) {
      throw new Error(
        `${type}.${field}はhttps、http、mailtoのURLを指定してください。fileや未知の形式は開けません。`,
      );
    }
  }
  ...
  if (type === "knowledge_node") {
    if (!knowledgeNodeTypes.has(input.node_type))
      throw new Error("knowledge_node.node_typeが不正です。");
    if (
    ...
    if (String(input.title || "").length > 200)
      throw new Error("knowledge_node.titleは200文字以内で入力してください。");
    if (String(input.body || "").length > 20000)
      throw new Error("knowledge_node.bodyは20000文字以内で入力してください。");
  }

  if (type === "ai_proposal") {
    if (!proposalSources.has(input.source)) throw new Error("ai_proposal.sourceが不正です。");
    if (!proposalPayloadTypes.has(input.payload_type))
      throw new Error("ai_proposal.payload_typeが不正です。");
  ...
  if (type === "project" && !projectStates.has(input.state))
    throw new Error("project.stateが不正です。");
  if (type === "capture_entry") {
    if (!captureEntryStates.has(input.state)) throw new Error("capture_entry.stateが不正です。");
    if (input.content_type === "audio" || input.kind === "voice_memo")
      validateAudioCaptureEntry(input);
    if (
      input.triaged_to_type != null &&
      input.triaged_to_type !== "" &&
      !entityRefTypes.has(input.triaged_to_type)
    ) {
  ...
  if (type === "task") {
    ...
    if (input.requester != null && !taskRequesters.has(input.requester))
      throw new Error("task.requesterが不正です。");
    if (input.intended_executor != null && !taskIntendedExecutors.has(input.intended_executor))
      throw new Error("task.intended_executorが不正です。");
    if (
      input.executor_identity != null &&
      input.executor_identity !== "" &&
      (typeof input.executor_identity !== "string" || input.executor_identity.length > 200)
    )
      throw new Error("task.executor_identityは200文字以内で入力してください。");
    if (input.work_state != null && !taskWorkStates.has(input.work_state))
      throw new Error("task.work_stateが不正です。");
    if (
      input.intended_executor === "ai_agent" &&
      input.state === "done" &&
      input.work_state !== "accepted"
    ) {
    ...
    for (const field of ["work_started_at", "work_reported_at"]) {
      if (
        input[field] != null &&
        input[field] !== "" &&
        Number.isNaN(new Date(input[field]).getTime())
      )
        throw new Error(`task.${field}が不正です。`);
    }
  ...
  if (type === "work_receipt") {
    if (typeof input.task_id !== "string" || !input.task_id.trim())
      throw new Error("work_receipt.task_idを入力してください。");
    if (!taskExecutorKinds.has(input.executor_kind))
      throw new Error("work_receipt.executor_kindが不正です。");
    if (
      typeof input.executor_label !== "string" ||
      !input.executor_label.trim() ||
      input.executor_label.length > 200
    )
      throw new Error("work_receipt.executor_labelは1〜200文字で入力してください。");
    if (
      typeof input.reported_at !== "string" ||
      Number.isNaN(new Date(input.reported_at).getTime())
    )
      throw new Error("work_receipt.reported_atが不正です。");
    if (
      input.started_at != null &&
      input.started_at !== "" &&
      Number.isNaN(new Date(input.started_at).getTime())
    )
      throw new Error("work_receipt.started_atが不正です。");
    ...
    if (
      ...
      (typeof input.source_session !== "string" || input.source_session.length > 200)
    )
  ...
  if (
    type === "theme" &&
    input.system_kind != null &&
    input.system_kind !== "" &&
    !themeSystemKinds.has(input.system_kind)
  ) {
    throw new Error("theme.system_kindが不正です。");
  }
  if (type === "schedule") {
    if (!scheduleOwnerTypes.has(input.owner_type))
      throw new Error("schedule.owner_typeが不正です。");
    if (!scheduleDateKinds.has(input.date_kind)) throw new Error("schedule.date_kindが不正です。");
    if (!scheduleConfidenceValues.has(input.confidence))
      throw new Error("schedule.confidenceが不正です。");
    if (!scheduleGranularityValues.has(input.granularity))
      throw new Error("schedule.granularityが不正です。");
    if (input.start_date && input.end_date && input.end_date < input.start_date) {
      throw new Error("schedule.end_dateはstart_date以降にしてください。");
    }
    if (
      input.range_semantics != null &&
      input.range_semantics !== "" &&
      !scheduleRangeSemantics.has(input.range_semantics)
    ) {
  ...
  if (type === "change_event") {
    if (!changeEventEntityTypes.has(input.entity_type))
      throw new Error("change_event.entity_typeが不正です。");
    if (!changeTypes.has(input.change_type))
      throw new Error("change_event.change_typeが不正です。");
    ...
    if (input.event_kind != null || input.entity_ref != null || input.occurred_at != null) {
      ...
      if (!ACTIVITY_EVENT_KINDS.includes(normalized.event_kind))
        throw new Error("change_event.event_kindが不正です。");
      if (!normalized.entity_ref?.type || !normalized.entity_ref?.id)
        throw new Error("change_event.entity_refが不正です。");
      if (Number.isNaN(new Date(normalized.occurred_at).getTime()))
        throw new Error("change_event.occurred_atが不正です。");
    }
  }
  if (type === "sketch") {
  ...
  if (type === "artifact") {
    ...
    if (
      ...
      input.generated_by !== "" &&
      !artifactGeneratedByValues.has(input.generated_by)
    ) {
      throw new Error("artifact.generated_byが不正です。");
    }
    if (
      input.file_size != null &&
      (!Number.isFinite(Number(input.file_size)) || Number(input.file_size) < 0)
    ) {
      throw new Error("artifact.file_sizeが不正です。");
    }
    const storageMode = input.storage_mode === "linked" ? "linked" : "managed";
    if (
      input.storage_mode != null &&
      input.storage_mode !== "" &&
      !artifactStorageModes.has(input.storage_mode)
    ) {
    ...
    if (
      ...
      input.link_status !== "" &&
      !artifactLinkStatuses.has(input.link_status)
    ) {
    ...
    if (
      ...
      input.export_format !== "" &&
      !["markdown", "pdf"].includes(input.export_format)
    ) {
...
export function normalizeEntity(type, input) {
  ...
  if (type === "reference")
    Object.assign(normalized, normalizeReferenceAssertion(normalized, { writeBoundary: true }));
  ...
  if (type === "working_copy" || type === "agent_session") {
    const safeRecord =
      type === "working_copy"
        ? normalizeWorkingCopy(normalized)
        : normalizeAgentSession(normalized);
    for (const key of Object.keys(normalized)) {
  ...
  if (type === "theme" || type === "project") {
    normalized.theme_charter = normalizeThemeCharter(normalized.theme_charter);
    normalized.theme_state = normalizeThemeState(normalized.theme_state);
  }
  for (const field of requiredFieldsForEntityType(type)) {
  ...
  if (type === "item") {
    normalized.schedule_status =
      normalized.planned_start || normalized.planned_end ? "scheduled" : "unscheduled";
    ...
    if (normalized.repeat_rule) {
      normalized.repeat_rule = {
        ...
        weekdays: Array.isArray(normalized.repeat_rule.weekdays)
          ? [...new Set(normalized.repeat_rule.weekdays.map((weekday) => Number(weekday)))].sort(
              (a, b) => a - b,
            )
          : undefined,
        month_day:
          normalized.repeat_rule.month_day == null || normalized.repeat_rule.month_day === ""
            ? null
            : Number(normalized.repeat_rule.month_day),
  ...
  if (type === "artifact") {
    ...
    if (typeof normalized.stored_path === "string")
      normalized.stored_path = normalized.stored_path.trim();
