...
function sessionId(input) {
  return text(input.session_id || input.sessionId || input.conversation_id || input.conversationId, 500);
}
...
export function normalizeAgentHookEvent(clientKind, input, now = () => new Date().toISOString()) {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("hook inputはJSON objectが必要です。");
  ...
  if (name === "userpromptsubmit" || name === "userpromptsubmitted" || name === "beforesubmitprompt") {
  ...
  if (name === "stop" || name === "agentstop" || name === "afteragentresponse") {
    return {
      ...base,
      kind: "progress",
      outcome: text(input.last_assistant_message || input.lastAssistantMessage || input.text || input.response, 8000),
      reason: text(input.status || input.stop_reason || input.stopReason, 200),
    };
  }
  if (name === "sessionend") {
    const reason = text(input.reason || input.final_status || input.finalStatus, 200) || "session_end";
...
function stateDirectory(options = {}) {
  return options.stateDirectory || path.join(resolveTaskenUserDataPath(options), "agent-session-observations");
}
...
async function withStateLock(filePath, callback) {
  ...
  throw Object.assign(new Error("session observationの排他待ちがtimeoutしました。"), { code: "OBSERVATION_LOCK_TIMEOUT" });
}
...
function mergeEvent(current, event) {
  ...
  if (event.kind === "start" && event.observed_at < state.started_at) state.started_at = event.observed_at;
  if (event.intent && (!state.intent || !state.intent_observed_at || event.observed_at < state.intent_observed_at)) {
    state.intent = event.intent;
    state.intent_observed_at = event.observed_at;
  }
  if (event.outcome && (!state.last_outcome || !state.last_outcome_observed_at || event.observed_at >= state.last_outcome_observed_at)) {
...
async function submitState(state, options = {}) {
  ...
  const workingCopyIds = context?.working_copies?.length === 1 ? [context.working_copies[0].id] : [];
  const inferredThemeIds = context?.themes?.length === 1 ? [context.themes[0].id] : [];
  const themeIds = splitIds(options.env?.TASKEN_AGENT_SESSION_THEME_IDS || process.env.TASKEN_AGENT_SESSION_THEME_IDS);
  const taskIds = splitIds(options.env?.TASKEN_AGENT_SESSION_TASK_IDS || process.env.TASKEN_AGENT_SESSION_TASK_IDS);
  ...
  return client.proposeAgentSession({
    ...
    ...(themeIds.length || inferredThemeIds.length ? { theme_ids: themeIds.length ? themeIds : inferredThemeIds } : {}),
