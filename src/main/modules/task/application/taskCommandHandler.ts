...
function attachCreationProvenance(
  ...
  if (!isCreate)
    throw new ApplicationCommandError(
      "INVALID_PAYLOAD",
      "provenanceはCreateTaskだけに指定できます。",
    );
...
export const taskApplicationCommandNames = [
  "CreateTask",
  "UpdateTask",
  "DeleteTask",
  "CompleteTask",
...
export interface TaskCommandRuntime {
  ...
  createEvent(
    ...
    entityId: string,
    kind: "created" | "updated" | "completed" | "rescheduled" | "deleted",
    before: Entity | null,
    after: Entity,
  ): Entity;
...
export class TaskCommandHandler {
  ...
  execute(command: CommandEnvelope): CommandReceipt {
    if (!this.handles(command.name))
      throw new ApplicationCommandError(
        "INVALID_ENVELOPE",
        `Task moduleが所有しないCommandです: ${command.name}`,
      );
  ...
  private saveTask(command: CommandEnvelope, isCreate: boolean): CommandReceipt {
    ...
    if (!isCreate && !current)
      throw new ApplicationCommandError("NOT_FOUND", "更新対象のTaskがありません。", {
        id: taskId,
      });
    ...
    if (
      ...
      (current.state === "done" || task.state === "done")
    ) {
      throw new ApplicationCommandError(
        "INVALID_TRANSITION",
        "完了状態の変更はCompleteTask/ReopenTaskを使用してください。",
      );
    }
    if (
      !isCreate &&
      current &&
      Object.prototype.hasOwnProperty.call(inputTask, "work_state") &&
      currentTaskWorkState(current) !== currentTaskWorkState(task)
    ) {
      const setupTransition =
        (currentTaskWorkState(current) === "not_delegated" &&
          currentTaskWorkState(task) === "ready_for_agent") ||
        (currentTaskWorkState(current) === "ready_for_agent" &&
          currentTaskWorkState(task) === "not_delegated");
      if (!setupTransition)
        throw new ApplicationCommandError(
          "INVALID_TRANSITION",
    ...
    const event = this.runtime.annotateEvent(
      ...
      attachCreationProvenance(
        ...
        this.runtime.createEvent(
          ...
          "task",
          taskId,
          taskChangeType(current, task, command.name),
          current,
          task,
        ),
        isCreate,
  ...
  private transitionTask(command: CommandEnvelope, completing: boolean): CommandReceipt {
    ...
    if (!current)
      throw new ApplicationCommandError("NOT_FOUND", "対象Taskがありません。", { id: taskId });
      ...
      ? {
          ...
          completed_at:
            command.source === "mobile"
              ? new Date(command.issuedAt).toISOString()
              : this.runtime.now(),
          completion_note:
            (command.payload as { completionNote?: string | null }).completionNote ??
            current.completion_note ??
            null,
        }
    ...
    const scheduleEvent = normalizedSchedule
      ? this.runtime.annotateEvent(
          ...
          this.runtime.createEvent(
            ...
            taskId,
            "rescheduled",
            this.repository.get("schedule", normalizedSchedule.id, true),
            normalizedSchedule,
          ),
        )
      : null;
    const operations: SaveOperation[] = [
      ...
      ...(normalizedSchedule
        ? [{ action: "save" as const, type: "schedule" as const, entity: normalizedSchedule }]
        : []),
      { action: "save", type: "change_event", entity: event },
      ...(scheduleEvent
        ? [{ action: "save" as const, type: "change_event" as const, entity: scheduleEvent }]
        : []),
  ...
  private deleteTask(command: CommandEnvelope): CommandReceipt {
    ...
    const current = this.repository.get("task", taskId);
    if (!current)
      throw new ApplicationCommandError("NOT_FOUND", "削除対象のTaskがありません。", {
        id: taskId,
      });
    if (!this.runtime.hasExpectedVersion(command, "task", taskId)) {
