...
async function runSmokeRestartTest(window: BrowserWindow): Promise<void> {
  ...
  `) as { artifactId: string; playable: boolean; microphoneArtifactId: string; microphonePlayable: boolean; importedVideo: { artifactId: string; ownerLinked: boolean; canPlay: boolean; seeked: boolean; metadata: boolean }; screenRecording: { artifactId: string; ownerLinked: boolean; canPlay: boolean; seeked: boolean; metadata: boolean } };
  ...
  const passed = renderer.playable && rangeVerified && renderer.microphonePlayable && microphoneRangeVerified
