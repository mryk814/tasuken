...
const contentDetailReadError = <Key extends "note_id" | "conversation_id" | "artifact_id">(
  key: Key,
) =>
  z
    .object({
...
const captureImageNotFoundSchema = z
  .object({
    error: z
      .object({
        code: z.literal("not_found"),
...
const noteDetailResponseSchema = z
  .object({
...
export const getArtifactMetadataRequestSchema = z
  .object({
...
export const getCaptureImageRequestSchema = z
  .object({
...
const captureImageDetailResponseSchema = z
  .object({
    image: captureImageDetailSchema,
    read_only: z.literal(true),
    ai_audience: z.literal("coding_agent"),
    next_tools: z.array(nextToolSchema).max(4),
  })
...
export const getArtifactMetadataResponseSchema = z.union([
  artifactDetailResponseSchema,
  artifactNotFoundSchema,
]);
export const getCaptureImageResponseSchema = z.union([
