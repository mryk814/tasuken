...
const CORE_ERROR_GUIDANCE = Object.freeze({
  ...
  CORE_REQUEST_FAILED: { retryable: true, next_action: "Taskenの状態を確認して再試行してください。" },
  ...
  UNSUPPORTED_MEDIA_TYPE: { retryable: false, next_action: "Content-Typeにapplication/jsonを指定してください。" },
  METHOD_NOT_ALLOWED: { retryable: false, next_action: "endpointで許可されたHTTP methodを使用してください。" },
  NOT_FOUND: { retryable: false, next_action: "endpointまたは対象IDを確認してください。" },
  IDEMPOTENCY_CONFLICT: { retryable: false, next_action: "内容を変える場合は新しいidempotency_keyを使用してください。" },
  CONFLICT: { retryable: false, next_action: "tasken.get_task_contextで対象を再取得し、最新versionでProposalを作り直してください。" },
  STALE_VERSION: { retryable: false, next_action: "tasken.get_task_contextで対象を再取得し、最新versionでProposalを作り直してください。" },
  PROPOSAL_TOO_LARGE: { retryable: false, next_action: "Proposalを64KiB未満の小さな単位に分割して再送してください。" },
...
export function taskenCoreErrorGuidance(code, explicit = {}) {
  ...
  return {
    ...
    next_action: typeof explicit.next_action === "string" && explicit.next_action
