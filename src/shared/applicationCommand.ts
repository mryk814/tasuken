...
export type ApplicationCommandSource =
  ...
  | "mobile"
  | "mcp";
export const applicationCommandSources = ["main_ui", "today_window", "quick_capture", "inbox", "command_palette", "tasken_root", "mobile", "mcp"] as const;
...
export class ApplicationCommandError extends Error {
  ...
  constructor(code: ApplicationCommandErrorCode, message: string, details?: Record<string, unknown>) {
    super(message);
...
export function parseCommandEnvelope(value: unknown): CommandEnvelope {
  if (!isRecord(value)) throw new ApplicationCommandError("INVALID_ENVELOPE", "Command envelopeが不正です。");
  const name = value.name;
  if (typeof name !== "string" || !applicationCommandNames.includes(name as ApplicationCommandName)) {
  ...
  if (!applicationCommandSources.includes(source)) throw new ApplicationCommandError("INVALID_ENVELOPE", "Command sourceが不正です。");
  const expectedVersions = value.expectedVersions;
  if (expectedVersions !== undefined && (!Array.isArray(expectedVersions) || expectedVersions.some((item) => {
    if (!isRecord(item)) return true;
    return typeof item.type !== "string" || !entityTypes.includes(item.type as EntityType) || typeof item.id !== "string" || !Number.isInteger(item.version);
  }))) {
  ...
  for (const expected of (expectedVersions || []) as ExpectedVersion[]) {
    if (expected.version < 0) throw new ApplicationCommandError("INVALID_ENVELOPE", "expected versionは0以上である必要があります。");
    const key = `${expected.type}:${expected.id}`;
    if (expectedKeys.has(key)) throw new ApplicationCommandError("INVALID_ENVELOPE", "expectedVersionsに重複があります。", { key });
    expectedKeys.add(key);
  }
  if (!isRecord(value.payload)) throw new ApplicationCommandError("INVALID_PAYLOAD", "Command payloadが不正です。");
  ...
  if (name === "CompleteTaskWithLearning") {
    ...
    for (const field of required) {
      ...
      if (!isRecord(entity) || typeof entity.id !== "string" || !entity.id.trim()) {
        throw new ApplicationCommandError("INVALID_PAYLOAD", `${name}の${field} payloadが不正です。`);
      }
    }
  ...
  if (name === "ApplyAiProposal") {
    if (!isRecord(value.payload.proposal) || typeof value.payload.proposal.id !== "string" || !value.payload.proposal.id.trim()
    ...
    for (const candidate of value.payload.candidates) {
      if (!isRecord(candidate) || typeof candidate.type !== "string" || !isRecord(candidate.entity)
  ...
  if (name === "EndFocusSession") {
    if (!isRecord(value.payload.session) || typeof value.payload.session.id !== "string" || !value.payload.session.id.trim()
      || typeof value.payload.completeTask !== "boolean") {
      throw new ApplicationCommandError("INVALID_PAYLOAD", "EndFocusSessionのpayloadが不正です。");
    }
    for (const field of ["task", "selectedNote", "promotedNote", "promotedReference", "nextTask", "statusUpdate"]) {
      const entity = value.payload[field];
      if (entity !== undefined && entity !== null && (!isRecord(entity) || typeof entity.id !== "string" || !entity.id.trim())) {
  ...
  if (name === "CommitTrimmedVideoArtifact") {
    for (const field of ["artifact", "reference"] as const) {
      const entity = value.payload[field];
      if (!isRecord(entity) || typeof entity.id !== "string" || !entity.id.trim()) {
        throw new ApplicationCommandError("INVALID_PAYLOAD", `CommitTrimmedVideoArtifactの${field} payloadが不正です。`);
      }
    }
  }
  ...
  if (name === "CreateTaskFromCapture") {
    if (typeof value.payload.captureId !== "string" || !value.payload.captureId.trim()
      || typeof value.payload.captureVersion !== "number" || !Number.isInteger(value.payload.captureVersion) || value.payload.captureVersion < 0
      || value.payload.transition !== "triage_to_task"
      || (value.payload.artifactIds !== undefined && (!Array.isArray(value.payload.artifactIds)
        || value.payload.artifactIds.some((id) => typeof id !== "string" || !id.trim())))) {
      throw new ApplicationCommandError("INVALID_PAYLOAD", "CreateTaskFromCaptureのcapture transitionが不正です。");
    }
  }
  if (value.payload.references !== undefined && (!Array.isArray(value.payload.references)
  ...
  if (Object.prototype.hasOwnProperty.call(value.payload, "operations")) {
    throw new ApplicationCommandError("INVALID_PAYLOAD", "汎用SaveOperationはCommand payloadへ渡せません。");
  }
  return {
    ...
    actor: { kind: actor.kind as CommandActor["kind"], id: typeof actor.id === "string" ? actor.id : undefined },
    source,
