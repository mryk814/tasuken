...
- 受け取った差分の親Revisionが、いま持っているheadと一致するときは**後継Revisionとして適用する**。
  Work Receiptはローカル保存ではappend-onlyだが、正本が公開した後継Revision（Taskの削除→復元などで`deleted_at`が変わる）は取り込む。
  内容が実質同じ再公開では書き換えず、headだけを進める。
  ここで失敗を返すと、その端末は同じ差分で**永久に停止する**（2026-09-26の実機NASで2週間停止した）。
...
- 送信側のTaskenが起動して同期するたび、欠けた差分ファイルは自動で書き戻される（10秒間隔の同期ごと）。「差分を再公開」は手動で今すぐ直したいとき用であり、通常は待てば直る。
