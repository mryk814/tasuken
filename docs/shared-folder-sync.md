...
- 共有フォルダの中身を誤って消してしまい、参加側で「同期差分 00000000000X を待っています」が解消しない場合は、データを持つホスト側のSettings「端末間同期」から「差分を再公開」を実行する。再公開はローカルOutboxの全履歴から欠けた差分ファイルを書き戻すため、途中からしか残っていない共有フォルダでも新規参加できるようになる。再公開後は共有フォルダの同期完了を待って参加側で「今すぐ同期」を実行する。
