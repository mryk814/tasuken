...
microphone 録音・文字起こし・画面録画はこの段階に含めない。

## 正本と確定順序

音声の所有関係は Artifact の `source_type=capture_entry` と `source_id` が正本であり、同じ関係を Reference へ重複保存しない。
...
OS path、音声bytes、receipt内のmanaged pathはworkspace load、entity list/get、変更通知へ含めない。
...
`CommitAudioCapture`、音声形式、Voice Capture表示だけが音声固有である。
画面録画は同じ共通基盤を再利用し、音声commandやUIへ条件分岐を追加しない。
