...
| GitHub Actions / Windows | Windows ABIの再構築、NSIS / portable生成、packaged smoke、MCP smoke |
...
5. `Windows release` workflowが同じタグのcommitをcheckoutし、`npm ci` → `npm run release:check`を実行する。成功するとGitHub Releaseが作成または更新される。
...
- packageに失敗した場合は、同じcommitでWSLの`npm run ci`を再実行し、Windows runnerのworkflowログでWindows固有の失敗（native module、installer、packaged smoke）を確認する。
