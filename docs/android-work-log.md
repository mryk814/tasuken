...
Mainは送信前と応答後に、Note→Theme→全体既定の公開policyで`external_ai`への許可を確認する。非許可ならproviderを呼ばず、応答待ち中に許可が撤回された場合も整理案を返さない。本人向けのMobile閲覧はこの公開policyとは独立している。
...
`mobile-work-log-gateway.test.mjs`は公開policyの継承・明示private・送信前拒否時のprovider未呼び出し・応答待ち中の許可撤回を検証する。
