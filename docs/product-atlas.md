...
| Agent Desk | 任せた仕事の進みと待ちを確認する  | AiProposal / WorkReceipt / Task | Review + Queue        | Feedへ集約（2026-09-26）。独立画面は無く、route ID `ai-io` は旧URLの受け皿 |
| Feed       | 変化と判断を流し読みする          | 既存canonicalからのprojection   | Attention projection  | 試作中 / Experimental（#604。右レールと「対応待ち」を持つ）                |
