...
時刻がある記録の順序は発生時刻の瞬間、次にstable event IDで確定する。
降順でも同じ二つの基準を反転する。
同一timestampに複数eventがあっても、次ページへ進む際に飛ばさない。

振り返りの作業記録は実施日のday精度を保つ。
timestampで期間を指定した場合も、そのtimezoneで境界日と重なる日単位の記録を含める。
その記録が指定した瞬間に行われたとは断定せず、`recall.date_basis = performed_day`と空の`local_time`を返す。
日単位の記録がある場合は暦日でまとめ、同日内では日単位の記録を先、時刻のある記録を後に並べる。
日単位の記録同士はstable ID順であり、実際の作業順を表さない。
