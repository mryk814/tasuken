...
| 2026-09-21 | token交換が `status 400` / `oauth_error: invalid_request` で失敗 | 当時は「client IDがWeb アプリ種別で、PKCEだけでは交換できない」と結論したが、**2026-09-26に誤りと判明**（下段）。真因は**デスクトップ アプリ種別でもsecretが必要**なことだった                                                                                                                                                                    |
...
| 2026-09-26 | 接続は成功するがDebriefの時間軸に予定が出ない                    | **ActivityLogPanelが、その日のTasken Activityが0件だと空の時刻軸だけを描いていた**（`hasStructuredActivity` がカレンダー予定を見ていない）。予定があれば時刻軸へ描くよう修正した（下記）                                                                                                                                                          |

### カレンダーだけの日も時刻軸へ描く（2026-09-26修正）

`ActivityLogPanel` は「TaskenのActivityがあるか」で時刻軸と要約一覧を切り替えていた。予定しかない日は
Activityが0件なので**空の時刻軸**が出て、接続済みでも予定が消えたように見えていた。
`hasTimelineItems`（Activityまたはカレンダー予定）で切り替え、予定を時刻軸へ描く。
`tests/activity-calendar-panel.test.mjs` の「calendar-only days draw the event timeline instead of the empty grid」が固定する。
実接続smokeの「Debriefの時間軸」も予定行数を実測する（`Activity`面は現行の`Debrief`へ改称済み）。
...
| token更新      | 1時間を超える利用での refresh_token による更新（secret付きrefreshの実測）      |
| 複数カレンダー | 初期実装はprimary calendarのみ。複数選択は別slice                              |
| offline        | 実回線断でのキャッシュ表示（状態の判定と文言はmockで確認済み）                 |
| 権限の最小化   | 実接続後に不要scopeを削減できるかの確認                                        |

## 実接続で確認できたこと（2026-09-26）

デスクトップ アプリ種別のclient ID（指紋 `a893ef9f`）と `TASKEN_GOOGLE_CLIENT_SECRET` で
