...
1. Debriefを開くと、選択日のActivity時系列を最初に表示する。
   フォーカス作業は、セッションNoteの開始から終了までを1件の期間として表示する。
   開始・自動保存・終了と明示的に関連付く完了・現在地更新を個別には数えず、元のChangeEventは保持する。
   進行中は「進行中」と表示し、日をまたぐセッションは各日の範囲に分けて表示する。
...
フォーカスを開始すると、タスク名・経過時間・終了操作を中心に表示する。
画面を閉じてもセッションは続き、メモ・関連資料は必要なときに開ける。
終了時はタスクも完了するかを選び、作業概要と次のタスクは任意で残す。
集約には既存のNote ID、Reference、終了コマンドのIDを使い、保存形式は変更しない。
...
Agent Deskで人が採用した時だけReport Noteが作成され、`properties_json.daily_report = { date: report_date }`が保存される。
