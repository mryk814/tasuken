...
| 場面             | 証跡（実測）                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 残っている境界                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| 基本の一往復     | `tests/agent-roundtrip-acceptance.test.mjs`（実SQLite＋実stdio MCP＋実mobile gateway。採用では`state==="todo"`のまま、明示完了で`done`）。**2026-09-26に実クライアント（Codex CLI 0.155.1 → 実Tasken MCP）でも一往復を実測**（`list_agent_ready_tasks`→`get_task_context`→`start_task_work`→`append_work_receipt`→`report_task_blocked`→実機Androidの回答→`get_task_context`で`human_reply`取得→`report_task_done`→Desktopで採用。採用後も`state==="todo"`） | —（実クライアント接続を実測）                                                        |
...
| 回答のMCP越しの受け渡し | 実stdio MCP → Desktop Core → Android回答 → MCP再取得までを通した（`tests/agent-roundtrip-acceptance.test.mjs`）。**2026-09-26に実クライアント（Codex CLI）でも実測**した（blocked報告→実機Androidの回答→`get_task_context`の`human_reply`取得。`provenance.reported_via = "mobile"`）。残るのは他プロバイダ・他クライアントでの確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Android                 | Gatewayの `/v1/attention` / `/v1/agent-replies`、Core側の読み出し、AndroidのRoomキャッシュ（`attention_cache` / `attention_state` / `pending_agent_reply`）と回答の再送まで実装済み（#601。`tests/mobile-agent-attention.test.mjs` / `MobileAttentionRepositoryTest` / `MobileAgentReplyContractTest`）。要対応の一覧・回答欄、Foldの詳細ペイン、新着の知らせ（アプリ内表示と任意のOS通知・deep link）は `AgentDeskAttentionUiTest` / `AttentionNotificationStoreTest` / `MobileEntryRequestTest` で確認済み（展開幅の目視は `output/android-attention/fold-attention-detail.png`）。**2026-09-26に実機SM-F966Q（v0.1.69）で実測**した: Fold展開幅の詳細ペインでの回答（回答後 `対応待ち 0 / 作業中 1`）、成果報告後の `成果確認` 出現、channel `tasken_attention_updates` のOS通知（`Tasken: 対応待ちが1件あります`＋見出しのみ）が実表示され、通知タップで `tasken://attention/<判断ID>` から該当詳細ペインが開いた。未取得時は「要対応をまだ取得できていません。」、取得後は「対応待ちはありません。」と区別する。**残る境界は、通知設定が既定オフ（左のチェックボックスのみが対象）であることと、多数の要対応が溜まったときの見え方** |
