...
| 報告だけ採用     | `tests/task-work-receipts.test.mjs`、`tests/agent-work-state.test.mjs`（`accepted_continuing`）                                                                                                                                                | 「受入れ済み／Taskは継続」の文言そのもののassert                                   |
| 採用して完了     | `tests/task-work-receipts.test.mjs`（採用→古い版での完了は競合→完了だけ再試行。Receiptは1件のまま）、`tests/agent-desk.test.mjs`（2 Command分割と失敗時の案内）                                                                                | Desktopのボタン自体を通す受け入れ                                                  |
| 差戻し           | `tests/mobile-gateway-phase4a.test.mjs`（`ready_for_agent`と`work_review_note`）、`tests/mcp-task-context.test.mjs`（理由をagentが取得）、`tests/agent-work-state.test.mjs`（新成果まで確認待ちに戻らない）、Android `WorkReceiptDetailUiTest` | Desktop UIからの差戻し実行                                                         |
| 再割当           | `tests/agent-work-state.test.mjs`（遅い報告は`past_attempt_report`）、`tests/agent-work-attempt.test.mjs`、`tests/agent-reply.test.mjs`                                                                                                        | 新しい作業単位での再委任のUI導線（未実装）                                         |
...
| 順不同           | `tests/agent-work-state.test.mjs`（`report_sequence`が到着順と発信時刻より優先）                                                                                                                                                               | 端末間の一致（golden fixtureが順不同の報告を含まない）                             |
| 質問とprogress   | `tests/agent-work-state.test.mjs`、`tests/attention-queue.test.mjs`（progress後も回答待ちが残る）                                                                                                                                              | —                                                                                  |
| 複数の判断       | `tests/attention-queue.test.mjs`（質問とNote変更案の合計2件、片方の処理で他方が残る）、`tests/agent-work-state.test.mjs`                                                                                                                       | Note変更案を同じTaskへ紐づける書き込み経路（`notes`は`request.task_id`を運ばない） |
