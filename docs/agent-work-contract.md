...
| 採用して完了     | `tests/agent-desk.test.mjs`（2 Command分割と失敗時の案内）、`smoke:proposal-live:focused`（採用のみ成功・古い版の回復）                             | 採用成功→完了失敗→完了だけ再試行の実行                                             |
| 差戻し           | `tests/mobile-gateway-phase4a.test.mjs`（`ready_for_agent`と`work_review_note`）、Android `WorkReceiptDetailUiTest`                                 | agent側の取得（MCP context）と、新成果まで未完了であることのassert                 |
