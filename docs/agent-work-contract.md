...
実装は `MobileAttentionDto.kt`（契約と射影）、`MobileLocalStore.kt`（`attention_cache` / `attention_state` / `pending_agent_reply`、DB v26）、`MobileGatewayRepository.kt`（取得と回答の再送）、`MainActivity.kt`（一覧・詳細ペインと回答欄）、`TodayViewModel`（`attention` と `agentReplyState`）。

検証は JVM の `MobileAgentReplyContractTest` / `MobileAttentionGoldenTest` と、実機相当の
`AgentDeskAttentionUiTest`（展開幅の詳細ペイン・競合時の入力保持・オフライン・未取得と0件の区別）。
