...
| `src/shared/applicationCommand.ts`               | `StartTaskWork.workAttemptId` の検証                     |
| `src/main/services/applicationCommandService.ts` | Taskの現在参照の更新、Receiptへの引き継ぎ                |
| `src/main/repositories/domain.mjs`               | 保存時の形式検証（UUID・範囲）                           |
...
| 未確認           | 内容                                                                                                                  |
| ---------------- | --------------------------------------------------------------------------------------------------------------------- |
| 人間の返答の保存 | `human_reply` Receiptを書く経路は未実装（#597）。`answered_resume_waiting` は契約として固定しただけで、まだ到達しない |
| 再割当のUI操作   | 委任解除と新しい作業単位の作成を利用者が実行する導線は未実装（#598 / #602）                                           |
| 表示             | `AgentWorkReadModel` を描画する画面はまだ無い（#596 / #599）                                                          |
| Android          | Kotlin側のDTOとgolden fixtureは未接続（#601）                                                                         |
