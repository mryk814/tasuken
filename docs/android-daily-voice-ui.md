...
### 2026-09-06 音声継続・複数Task・日時の確認

更新時刻とWork Receiptの報告時刻は、保存済みISO timestampを端末のtime zoneへ変換して表示する。日本時間なら`2026/9/5 9:00 JST`のように表示し、予定の日付そのものには時差変換をかけない。

Android単体124件、S23で整理UI・整理通信・Task詳細・Outbox・Draft保存の71件が成功。スクリーンショットを追加した整理UI／Task詳細12件も成功した。複数案の表示・編集・除外、保存途中の失敗で全件を戻して再試行する経路、整理案の日付と原文を保持する保存、JST表示を確認した。S23 Debugへ検証版を反映した。

既存エミュレーター（2076×2152、390dpi）ではinstrumentation起動中にANR終了。OSログには98% CPU使用（92% kernel）も記録され、同じAPKがS23で成功したが、エミュレーターでの完了・目視確認は未達。実発話を用いた長い間の認識継続とLLMの日付解釈は別途確認が必要。
...
- 音声認識は一度押して開始し、もう一度押して確定する。Android側が短い無音で認識結果を区切っても入力セッションを再開し、区切られた文字列を連結する。約30秒の無発話を認識区切りで確認したら確定するため、終了までの長さはサービスの通知タイミングで前後する。認識エラー時も聞き取れた文字を保持し、途中終了したことを表示する。再録音は「話し直して置き換える」で明示する。
...
一度の入力に独立した用件が複数あれば最大8件のTask案に分ける。先頭は従来どおり全項目を編集でき、2件目以降はタイトルを直すか不要なTaskを外してからまとめて追加する。
...
複数Taskは全件を同じローカルトランザクションで保存し、成功してから同期を予約する。途中で保存に失敗した場合は全件を戻して入力を保持する。各Taskの同期コマンドは個別に再試行される。
