...
- 上段に道具とUndo / Redoを固定し、下段に選んだ道具の設定を表示する。道具を切り替えてもキャンバスの位置を変えない。
- 図形は線・四角・楕円を直接選べ、ほかの図形はパレットから選ぶ。パレットはEscape、外側クリック、道具切替で閉じる。
- 文字サイズは数値、線幅・消しゴムは見本と現在のpx値で確認する。色・太さは従来どおり道具ごとに記憶する。
- 図形・矢印は描いた直後に選択状態になり、道具を持ち替えずに本体のドラッグで移動、右下の取っ手でリサイズできる。空いている場所からドラッグすると次を描く。選択中の図形の内側に重ね描きするときはEscapeで選択を解除する。過去の図形を選び直すときは「選択」を使う。
- 選択中の複製・削除・前後関係は可視ボタンから操作でき、削除はUndoで戻せる。

回帰確認: `npm run build` の後に `npm run smoke:sketch:focused`。パッケージ版は`npm run smoke:sketch:packaged`を使う。一時データで描画直後の移動・拡縮、連続描画、複製後のEscape、重ね描き、削除、Undo、保存後の再読み込みを確認する。
