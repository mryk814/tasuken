...
- 部分性格: **Feedは読み物・閲覧**。委任した仕事から得られた学びを読む面として、本文は既定より大きく取り、連続して読むことを優先する（`feed-surface.md`。2026-09-21に追加）
...
- 推論はDesktop側の共通サービスから呼び出し、Android GatewayとDesktop Quick CaptureのTask入力で利用する。接続情報はSettingsの「AI & Context」で指定し、APIキーは端末内でsafeStorageにより暗号化する。未保存時はDesktopプロセスの環境変数を利用する。AndroidやTaskのDB・ソースコードへキーを保存しない。
...
- Androidは機能の再現ではなく、その端末ならではの手軽さで使いたくなる機能だけを厳選する。Desktopでできることのうち、移動中・片手・短時間で価値が出るものを優先し、使わない機能や意味を説明できない入口は残さない。
- 何を使い、何を使わないかは操作・利用状況から診断できるよう記録する。搭載・廃止の判断は理由と日付を残し、次の取捨選択で参照する。
- 種類（Task / Capture）のように利用者が意味を共有できていない切り替えはUIに出さない。長文など内部で扱いが変わる場合も、利用者へ概念を要求せず自動で振り分ける。
...
4. Sketchの推論実行はCodex等の外部Agentへ委ねる。SketchにLLM APIキー、provider設定、model選択、streaming実行を持たない。Desktop・Android共通の入力整理だけは上記の限定API連携を例外とする。
