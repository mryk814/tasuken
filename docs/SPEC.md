...
### External Agent Context / Proposal

TaskenはNoteやThemeから外部Agentへ渡すContextを選択・Previewし、MCPまたはクリップボードで受け渡す。アプリ内ではLLM APIを呼び出さない。
外部Agentから返る変更案はNote IDと`base_version`を持つPending Proposalとして受け取り、Diff確認後にApplication Commandとcanonical Markdown保存を通して採用する。
旧`properties_json.draft_workspace`はNoteの未知フィールドとして保持するが、表示・追記する専用UIは持たない。
...
データの正本は各PC上のローカルデータベースとする。
複数PC間では、OneDriveまたは社内共有フォルダを搬送路として、端末別の変更差分を自動交換できる。
共有フォルダへSQLiteファイルそのものを置かず、各端末はオフラインでもローカル保存を継続する。

中央クラウドDBや常時接続サーバーは必須としない。
同期を使わないバックアップ・手動移行経路としてSnapshot Export / Importも維持する。

## 12.2 共有フォルダ差分同期

複数PCでの継続利用には以下の方式を採用する。

- 各PCではローカルDBを用いてアプリを利用する
- 保存transactionと同時に、送信待ちの変更差分をOutboxへ記録する
- 共有フォルダでは端末ごとに書き込み先を分離する
- 起動後は定期的に未送信差分を出力し、他端末の未受信差分を適用する
- 同一Entityの分岐変更は競合として保持し、利用者が残す内容を選ぶ
- 同期停止中や共有フォルダへ接続できない間もローカル保存を止めない

SQLiteなどのDBファイルそのものをクラウド同期フォルダに直接置き、複数PCから同時利用する方式は採用しない。
これは、ファイル同期の競合やDB破損のリスクを避けるためである。
同期形式と利用手順の正本は`docs/shared-folder-sync.md`とする。
