...
| 3    | 反応と会話を学びにつなぐ（おもしろい、自分の投稿、質問の引き継ぎ、返信）                 | 一部実装（返信の保存・削除、AIへの質問の読み出しまで。返答の取り込みと自分の投稿欄は未着手） |
...
質問が「回答あり」になり返答が同じスレッドで読めること・**終了して起動し直しても**
投稿とブックマークと保存したNoteと返信・返答が残ることを実測する。
