...
| 3    | 反応と会話を学びにつなぐ（おもしろい、自分の投稿、質問の引き継ぎ、返信）                 | 実装済み（おもしろいの保存、返信、AIへの質問と返答、自分の投稿欄） |
...
5. 外部AIは `tasken.get_feed_context`（読み出し専用）で、未回答の質問と元の投稿の抜粋、
   直近の投稿、利用者の明示的な反応（ブックマーク・おもしろい）を読む。
...
投稿とブックマークと保存したNoteと返信・返答・自分の投稿が残ることを実測する。
...
| おもしろいの活用 | 印は保存し、`get_feed_context` でAIへ渡すが、題材選びの調整は利用後に行う |
