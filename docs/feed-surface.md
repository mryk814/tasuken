...
| Agent Desk   | 任せた仕事の進み具合を確認する             | 回答、成果確認、報告採用、委任内容確認   | token数、モデル比較、実行コンソール、変更案の採否（Feedの「対応待ち」で行う） |
...
| 5    | TodayとAgent Desk                                                                           | 実装済み（TodayのAI非表示、ToDoの委任状態とFeedへの導線、Agent DeskのAIごとの入口、Feed「対応待ち」の提案確認） |
