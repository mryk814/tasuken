...
外部AIは `tasken.get_feed_context` で未回答の質問と元の投稿を読み、返答を返す。
スレッドの印は「AIに依頼済み」と「回答あり」を区別し、依頼しただけで回答が来たように見せない。
...
6. AIの返答（`author_kind: "ai"`、`author_label`）は同じスレッドへ並べられる形にしてある。
...
`tests/feed-context-mcp.test.mjs` は「AIに聞く」で残した質問が、実SQLite + 実Core + 実stdio MCPの
`tasken.get_feed_context` で読めること（未回答だけを既定で返す、元の投稿の抜粋と参照ID、
明示的な反応、抜粋の上限、読み出しが正式データを変えないこと、不正な引数の拒否）を確認する。
...
| AIの返答の取り込み | 質問の読み出しは実装済み。返答を送るMCP toolと回答の表示は未実装     |
