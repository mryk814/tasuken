...
- エンティティ操作は詳細→編集の2段階ドロワー、削除は編集ドロワー末尾のDanger Zone＋undoトースト、削除と状態変更を混同しない（§18）
- 一覧行の常時アクションは4個まで、二次操作はメニューへ（§18b）
- 右クリックは補助導線。すべての操作は可視UIから到達可能にする（§19）
- フィルタ状態は保存・復元し、0件と空状態を区別する（§22）
- トーストはtone明示引数（success/warning/danger/infoの意味固定、§23）
- カウントバッジは要対応の数のみ（§24）
- 一覧500行超は仮想化かページング、検索入力は200〜300msのdebounce（§25）
- フォームは送信時検証＋エラー欄のみblur再検証。placeholderをラベルにしない（§5c）
- 秘密情報（APIキー等）をコード・ログ・Exportに直書きしない
