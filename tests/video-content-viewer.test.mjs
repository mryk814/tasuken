...
import {
  ...
  videoOwnerThemeIsSaved,
} from "../src/renderer/src/features/workspace/videoArtifactView.ts";
...
test("video prepare requires the drawer Theme to be saved and commit is an explicit decision", () => {
  ...
  const pickBlock = source.slice(
    source.indexOf("async function pickVideo"),
    source.indexOf("async function discardPreparedVideo"),
  );
...
test("ArtifactSection exposes compact loading, empty, error, and success states without a permanent empty panel", () => {
  ...
  assert.match(
    source,
    /const videoState = videoLoading \? "loading" : videoError \? "error" :[\s\S]*\? "success" : "empty"/,
  );
...
test("Focus Session attaches video to the active focus_session Note identity", () => {
  ...
  assert.match(
    source,
    /session && \(\s*<ArtifactSection[\s\S]*sourceType="note"[\s\S]*sourceId=\{session\.id\}/,
  );
});
...
test("generic drag/drop rejects media by name before asking Electron for a raw path", () => {
  ...
  const block = source.slice(
    source.indexOf("async function importFiles"),
    source.indexOf("async function pickManagedFiles"),
  );
  assert.match(
    block,
    /filter\(\(file\) => !isDedicatedMediaFileName\(file\.name\)\)[\s\S]*pathForFile/,
  );
...
test("video viewer inspects by ID, keeps race cancellation, and gates external-open to verified/codec errors", () => {
  ...
  const editor = fs.readFileSync(
    "src/renderer/src/features/workspace/components/VideoTrimEditor.tsx",
    "utf8",
  );
  ...
  assert.match(
    source,
    /let cancelled = false[\s\S]*if \(!cancelled\) setLoad\(next\)[\s\S]*cancelled = true/,
  );
  assert.match(
    source,
    /mediaAvailability === "available" \|\| load\.mediaAvailability === "unsupported_codec"/,
  );
  assert.doesNotMatch(
    source,
    /mediaAvailability === "missing" \|\| load\.mediaAvailability === "changed"/,
  );
  ...
  assert.match(
    source,
    /const result = await workspaceApi\.openMediaArtifactExternal\(artifact\.id\)[\s\S]*if \(!result\.ok\) setToast\(result\.error[^\n]+"danger"\)/,
  );
...
test("managed video viewer has one combined trim bar and writes a derived artifact", () => {
  ...
  assert.match(
    viewer,
    /<VideoTrimEditor[\s\S]*?editable=\{load\.artifact\.storage_mode !== "linked"\}/,
  );
  ...
  assert.match(
    css,
    /\.video-trim-selection[\s\S]*?left: var\(--trim-start\)[\s\S]*?right: calc\(100% - var\(--trim-end\)\)/,
  );
});
