...
test("canonical Markdownは本文の先頭末尾空白を保持し、末尾改行だけを補う（#291）", () => {
  ...
  const contentWithNewline = buildCanonicalMarkdownContent({ title: "空白", body: bodyWithNewline });
...
test("atomic replaceは既存Markdownを置換し、一時ファイルを残さない（#291）", () => {
  ...
  try {
    ...
    assert.deepEqual(readdirSync(directory).filter((name) => /\.tmp$|\.bak$/.test(name)), []);
  } finally {
...
test("atomic replaceはbackup削除失敗でも新正本を保存済みとし、旧backupを残す（#291）", () => {
  ...
  try {
    ...
    const warning = writeAtomicTextFile(filePath, "新本文\n", "backup-warning-test", backupCleanupFailingFileSystem);
    assert.match(warning, /旧ファイルの退避を削除できませんでした/);
    assert.equal(readFileSync(filePath, "utf8"), "新本文\n");
    assert.equal(readFileSync(path.join(directory, ".note.md.backup-warning-test.bak"), "utf8"), "旧本文\n");
    assert.deepEqual(readdirSync(directory).filter((name) => /\.tmp$/.test(name)), []);
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
...
test("保存経路が正本Markdownの状態を持ち、外部変更を確認する（#291）", () => {
  ...
  assert.match(notesSource, /canonicalFileState === "external_change" && window\.confirm\(/);
