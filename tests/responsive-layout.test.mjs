...
test("sidebar collapse is accessible and persisted as device UI state", () => {
  ...
  assert.match(cssSource, /\.sidebar \{[^}]*overflow-x: hidden;\s*overflow-y: auto;/);
  assert.match(
    cssSource,
    /\.app-shell\.is-sidebar-collapsed \.sidebar \{\s*padding-inline: var\(--space-3\);/,
  );
  assert.match(
    cssSource,
    /\.app-shell\.is-sidebar-collapsed \.sidebar > \* \{\s*width: 100%;\s*min-width: 0;/,
  );
  assert.match(
    cssSource,
    /\.app-shell\.is-sidebar-collapsed \.sidebar \.nav-heading \{[^}]*overflow: hidden;\s*visibility: hidden;\s*white-space: nowrap;/,
  );
  assert.match(
    cssSource,
    /\.app-shell\.is-sidebar-collapsed \.theme-nav > button \{\s*justify-content: center;/,
  );
  assert.doesNotMatch(
    cssSource,
    /\.app-shell\.is-sidebar-collapsed \.utility-nav \{ margin-top: auto;/,
  );
});
...
test("custom titlebar keeps native window controls and lightweight display help menus", () => {
  ...
  assert.match(
    cssSource,
    /\.app-content-viewport > \.app-shell,[\s\S]*transform: scale\(var\(--app-content-zoom\)\);[\s\S]*transform-origin: top left;/,
  );
...
test("compact desktop layout protects the main work area", () => {
  ...
  assert.match(cssSource, /\.context-pane \{\s*display: none;/);
  assert.match(cssSource, /\.drawer \{\s*position: absolute;/);
  assert.match(cssSource, /\.notes-workbench \{[^}]*minmax\(0, 1fr\)/);
  assert.match(cssSource, /\.notes-page \{\s*width: min\(1600px, 100%\);/);
  assert.match(cssSource, /\.chat-refs-page \{\s*width: min\(1600px, 100%\);/);
  assert.match(
    cssSource,
    /\.sketch-library-page,[\s\S]*\.artifacts-page \{\s*width: min\(1600px, 100%\);/,
  );
...
test("Ctrl+Sは入力中でも開いているdrawerを保存し、ブラウザ既定動作を止める", () => {
  assert.match(
    workspaceSource,
    /\(event\.ctrlKey \|\| event\.metaKey\)[\s\S]*?event\.key\.toLowerCase\(\) === "s"\s*&&\s*drawerFormRef\.current/,
  );
...
test("titlebar content owns one bounded height contract without nested viewport overflow", () => {
  assert.match(cssSource, /\.app-shell \{\s*position: relative;\s*height: 100%;\s*min-height: 0;/);
  assert.match(cssSource, /\.notes-page \{[^}]*height: 100%;\s*min-height: 0;/);
  assert.match(cssSource, /\.drawer \{[^}]*height: 100%;\s*min-height: 0;/);
  assert.match(cssSource, /\.context-pane \{[^}]*height: 100%;\s*min-height: 0;/);
...
test("Sketch keeps the page rail separate from canvas tools and inside the titlebar viewport", () => {
  assert.match(
    cssSource,
    /\.app-shell\.is-canvas-route \.main-area \{[^}]*height: 100%;[^}]*overflow: hidden;/,
  );
  assert.match(
    cssSource,
    /\.sketch-page \{[^}]*grid-template-columns: var\(--sketch-page-rail-width\) minmax\(0, 1fr\);/,
  );
  assert.match(
    cssSource,
    /\.sketch-toolbar \{[^}]*grid-column: 2;[^}]*grid-row: 2;[^}]*overflow: visible;/,
  );
...
test("Notes controls wrap as groups without stretching their labels", () => {
  ...
  assert.match(
    cssSource,
    /\.notes-page \.header-actions > button \{[^}]*flex: 0 0 auto;[^}]*white-space: nowrap;/,
  );
  assert.match(
    cssSource,
    /\.notes-page > \.filter-bar \{[^}]*flex-wrap: wrap;[^}]*justify-content: flex-start;/,
  );
  assert.match(
    cssSource,
    /\.notes-page > \.filter-bar \.segmented button \{[^}]*flex: 0 0 auto;[^}]*white-space: nowrap;/,
  );
});
...
test("狭幅でも日本語ラベルを一文字ずつ縦積みにしない共通契約がある（#300）", () => {
  ...
  assert.match(
    cssSource,
    /\.primary-button,[\s\S]*?\.segmented button,[\s\S]*?\.badge,[\s\S]*?word-break: keep-all;\s*overflow-wrap: normal;/,
  );
  // ツールバーは詰め込まず行送りする。
  assert.match(cssSource, /\.toolbar-row \{\s*display: flex;\s*flex-wrap: wrap;/);
  assert.match(cssSource, /\.toolbar-row > \* \{\s*flex: 0 0 auto;/);
  // 見出しと操作が同じ行に収まらない場合も潰さず折り返す。
  assert.match(cssSource, /\.section-heading \{[^}]*flex-wrap: wrap;/);
  assert.match(
    cssSource,
    /\.inline-actions,\s*\.form-actions \{\s*display: flex;\s*flex-wrap: wrap;/,
  );
  // 列幅が狭い空状態のボタンも潰れない。
...
test("作業領域の内側にも不要な横スクロールを作らない（#300）", () => {
  ...
  assert.match(cssSource, /\.header-actions \{\s*display: flex;\s*flex-wrap: wrap;/);

  // 段組みの切り替えはウィンドウ幅ではなく実際のpanel幅を基準にする。
  assert.match(cssSource, /\.page \{\s*container: page \/ inline-size;/);
  assert.match(
    cssSource,
    /@container page \(max-width: 760px\) \{[\s\S]*?\.sketch-library-row \{\s*grid-template-columns: 1fr;/,
  );
  assert.match(
    cssSource,
    /@container page \(max-width: 720px\) \{[\s\S]*?\.settings-form label \{\s*grid-template-columns: minmax\(0, 1fr\);/,
  );
