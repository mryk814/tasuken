...
const { buildRecallPaletteEntries } = await import(
    ...
    tasks: [
      {
        id: "t1",
        title: "測定を進める",
        description: "試料のオリオン濃度を確認",
        state: "doing",
        priority: "normal",
        project_id: "theme-hidden",
        checklist_items: [
          { id: "check-1", title: "シリウス検量線を作る", done: false, sort_order: 0 },
        ],
      },
    ],
    notes: [
  ...
  const artifacts = [
    {
      id: "a1",
      title: "結果",
      filename: "polaris-results.csv",
      stored_path: "/managed/vega/result.csv",
      original_path: null,
      target: null,
      source_type: "task",
      source_id: "t1",
...
test("Recall Paletteは本文・待ち・記録・Knowledge・ChatをWorkspace横断検索する", () => {
  ...
  for (const [query, expected] of cases) {
    assert.deepEqual(
      filterCommandEntries(entries, query).map((entry) => entry.id),
      [expected],
      query,
    );
  }
...
test("Recall Paletteは全Theme投影と既存画面へのtargetを返す", () => {
  ...
  assert.deepEqual(byId.get("plan:p1").target, {
    kind: "drawer",
    route: "timeline",
  ...
  assert.deepEqual(byId.get("note:n1").target, {
    kind: "drawer",
    route: "notes",
    entityType: "note",
    entityId: "n1",
  });
  assert.deepEqual(byId.get("waiting:w1").target, {
    kind: "drawer",
    route: "waiting",
    entityType: "waiting",
    entityId: "w1",
  ...
  assert.deepEqual(byId.get("capture:c1").target, {
    kind: "drawer",
    route: "inbox",
  ...
  assert.deepEqual(byId.get("knowledge:k1").target, {
    kind: "drawer",
    route: "knowledge",
  ...
  assert.deepEqual(byId.get("chat:r1").target, {
    kind: "drawer",
    route: "chat-refs",
    entityType: "resource",
    entityId: "r1",
  ...
  assert.deepEqual(byId.get("resource:r2").target, {
    kind: "drawer",
    route: "notes",
    entityType: "resource",
    entityId: "r2",
...
test("Ctrl+K契約はPaletteへ一本化し、入力中の既存ショートカットを守る", () => {
  ...
  const palette = readFileSync(
    "src/renderer/src/features/workspace/components/CommandPalette.tsx",
    "utf8",
  );
  ...
  assert.match(
    app,
    /const pending = drawerAutosavePromise\.current[\s\S]*?await pending[\s\S]*?isDrawerFormDirty\(\)/,
  );
  assert.match(
    app,
    /const submittedSignature = formSignature\(form\)[\s\S]*?drawerFormInitialSignature\.current = submittedSignature/,
  );
  assert.match(
    app,
    /recalledGroup[\s\S]*?setActiveGroups\(\[\.\.\.activeGroups, recalledGroup\]\)/,
  );
  ...
  assert.match(
    styles,
    /\.command-palette-result-status\s*\{[\s\S]*?color: var\(--color-text-secondary\)/,
  );
