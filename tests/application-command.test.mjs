...
test("DeleteTask uses the same expected-version boundary and keeps deletion undoable", () => {
  ...
  const replay = new ApplicationCommandService(repo).execute(envelope("DeleteTask", { taskId: "delete-task" }, "delete-task-command", [{ type: "task", id: "delete-task", version: 1 }]));
  assert.deepEqual(replay, receipt);
  assert.equal(repo.list("change_event").filter((event) => event.command_id === "delete-task-command").length, 1);
