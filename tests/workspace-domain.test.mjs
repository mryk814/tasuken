...
const adapter = await importBundled(
  "src/renderer/src/features/workspace/domain-model/compat/legacyAdapter.ts",
);
...
const taskDuplication = await importBundled(
  "src/renderer/src/features/workspace/domain-model/taskDuplication.ts",
);
const timelineProjection = await importBundled(
  "src/renderer/src/features/workspace/domain-model/compat/timelineProjection.ts",
);
...
test("workspace domain keeps pending AI proposals for the Sidebar action badge", () => {
  const domain = adapter.buildWorkspaceDomain(
    workspace({
      ai_proposals: [
  ...
  assert.deepEqual(
    domain.ai_proposals.map((proposal) => proposal.id),
    ["proposal-pending", "proposal-accepted"],
  );
  assert.equal(domain.ai_proposals.filter((proposal) => proposal.status === "pending").length, 1);
});

test("workspace domain keeps canonical Note project_id and only falls back for legacy rows", () => {
  const domain = adapter.buildWorkspaceDomain(
    workspace({
      notes: [
        { id: "canonical", title: "Canonical", project_id: "theme-a" },
        { id: "legacy", title: "Legacy", theme_id: "theme-b" },
        { id: "conflict", title: "Conflict", project_id: "theme-a", theme_id: "theme-b" },
        { id: "canonical-null", title: "No Theme", project_id: null, theme_id: "theme-b" },
        { id: "legacy-null", title: "Legacy personal", theme_id: null },
      ],
    }),
  );
  assert.deepEqual(
    domain.notes.map((note) => [note.id, note.project_id]),
    [
      ["canonical", "theme-a"],
      ["legacy", "theme-b"],
      ["conflict", "theme-a"],
      ["canonical-null", null],
      ["legacy-null", "theme-personal-default"],
    ],
  );
});

test("compat migration: legacy workspace conversion preserves first-class entity meanings", () => {
  const { workspace: domain, report } = adapter.legacyWorkspaceToDomainMigration(
    workspace({
  ...
  assert.equal(
    domain.waitings.find((waiting) => waiting.legacy_item_id === "status-waiting-1")?.state,
    "waiting",
  );
  assert.equal(
    domain.plan_nodes.find((node) => node.legacy_item_id === "milestone-1")?.type,
    "milestone",
  );
  assert.equal(domain.plan_nodes.find((node) => node.legacy_item_id === "period-1")?.type, "phase");
  assert.equal(
    domain.tasks.find((task) => task.legacy_item_id === "task-1")?.plan_node_id,
    "plan_node-period-1",
  );
  ...
  assert.match(
    adapter.formatMigrationReport(report),
    /Migration report\n----------------\nLegacy items: 7/,
  );

  const legacy = adapter.projectLegacyWorkspace(
    domain,
    workspace({
      source_records: [{ id: "source-1", source_type: "clipboard", raw_text: "raw" }],
    }),
...
test("domain view models keep task, waiting, capture, and timeline concerns separate", () => {
  const domain = {
    ...
    waitings: [{ id: "waiting-1", title: "waiting", waiting_for: "Lab", state: "waiting" }],
    plan_nodes: [
  ...
  assert.deepEqual(
    selectors.buildTodoView(domain).tasks.map((row) => row.task.id),
    ["task-1", "task-done"],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    selectors.buildWaitingView(domain).waitings.map((row) => row.waiting.id),
    ["waiting-1"],
  );
  assert.deepEqual(
    selectors.buildTodayView(domain, "2026-06-19").map((entry) => entry.type),
    ["task", "waiting", "milestone"],
  );
...
test("today view separates ongoing period tasks from explicit daily tasks", () => {
  const domain = {
    ...
    schedules: [
      ...
      {
        id: "schedule-overdue-period",
        owner_type: "task",
        owner_id: "task-overdue-period",
  ...
  assert.deepEqual(
    selectors.buildTodayView(domain, "2026-06-20").map((entry) => entry.task.id),
    ["task-end"],
  );
  ...
  assert.deepEqual(
    periods.map((row) => row.task.id),
    ["task-period", "task-start"],
  );
  assert.ok(
    periods.every((row) => row.unspecified),
    "意味未設定として区別できる",
  );
...
test("task duplication creates an editable new todo without completion state", () => {
  ...
  assert.deepEqual(
    duplicated.task.checklist_items.map((item) => ({
      title: item.title,
      done: item.done,
...
test("inbox view sorts untriaged captures by newest timestamp", () => {
  ...
  assert.deepEqual(
    selectors.buildInboxView(domain).entries.map((entry) => entry.id),
    ["cap-new", "cap-old", "cap-date-b", "cap-date-a"],
  );
  assert.deepEqual(
    selectors.buildMicroMemoView(domain).entries.map((entry) => entry.id),
    ["cap-memo"],
  );
});
...
test("micro memo can be sent back to the untriaged inbox lane", () => {
  ...
  const ops = persistence.buildSendMicroMemoToInboxOperations(memo, {
...
test("post-migration smoke: legacy items=[] still produces meaningful views and export", () => {
  ...
  assert.ok(
    exportData.items.length >= 4,
    `expected >= 4 export items from domain, got ${exportData.items.length}`,
  );
...
test("export data excludes notes explicitly removed from export target", () => {
    ...
    notes: [
      {
        ...
        theme_id: "theme-1",
        body_markdown: "共有する",
        properties_json: { export_enabled: true },
      },
      {
        id: "note-2",
        title: "下書き文書",
        theme_id: "theme-1",
        body_markdown: "まだ出さない",
        properties_json: { export_enabled: false },
      },
  ...
  assert.deepEqual(
    exportData.notes.map((note) => note.title),
    ["公開文書", "従来メモ"],
  );
  const markdown = io.exportMarkdown(exportData);
...
test("document publish flag is separate from AI export inclusion", () => {
  assert.equal(
    io.notePublishEnabled({ properties_json: { publish_enabled: true, ai_export_enabled: false } }),
    true,
  );
  assert.equal(io.notePublishEnabled({ properties_json: { ai_export_enabled: true } }), false);
  assert.equal(
    io.noteAiExportEnabled({
      properties_json: { publish_enabled: false, ai_export_enabled: true },
    }),
    true,
  );
  assert.equal(
    io.noteAiExportEnabled({
      properties_json: { publish_enabled: true, ai_export_enabled: false },
    }),
    false,
  );
});
...
test("post-migration smoke: MCP mergedItems includes plan_nodes", async () => {
  ...
  try {
    ...
    assert.equal(
      merged.length,
      3,
      `expected 3 merged items (task+waiting+plan_node), got ${merged.length}`,
    );
    assert.ok(
      merged.some((i) => i.kind === "milestone"),
      "merged should include milestone from plan_node",
    );
