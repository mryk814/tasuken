...
function fixture() {
  ...
  notes.push({ id: "note-hidden", title: "Private", body_markdown: "HIDDEN_BODY_SENTINEL", project_id: theme.id, ai_visibility: [], updated_at: now });
  const conversation = {
  ...
  const references = [
    ...notes.map((note, index) => ({ id: `ref-note-${index}`, source_type: "task", source_id: task.id, target_type: "note", target_id: note.id, relation_type: "context" })),
    ...
    { id: "ref-artifact", source_type: "task", source_id: task.id, target_type: "artifact", target_id: artifact.id, relation_type: "created_for" },
  ];
  const receipt = {
    id: "receipt-visible",
    task_id: task.id,
    ...
    summary: "普通の説明は保持。 token=TOP_SECRET C:\\Users\\private\\receipt.txt /home/private/receipt.txt https://bob:password@example.com/report?api_key=URL_KEY#x",
...
class FixtureRepository {
  ...
  list(type, includeDeleted = false) {
    ...
    const key = {
      theme: "themes", task: "tasks", repository_context: "repository_contexts", work_receipt: "work_receipts",
    }[type] || collection;
...
async function callMcp(coreClient, name, args, readContextProvider = () => { throw new Error("DB_CONSTRUCTOR_SENTINEL"); }) {
...
test("MCP Wave 3 get_task_context is exact across legacy, in-process, HTTP, and MCP", async () => {
  ...
  const legacy = new ReadOnlyTaskenContext("wave3.sqlite", { workspace, aiVisibilityDefault: ["coding_agent"] });
  try {
    await host.start();
...
test("Wave 3 preserves graph/text bounds and redacts receipt, URL, and local path secrets", () => {
  const workspace = fixture();
  const legacy = new ReadOnlyTaskenContext("wave3-security.sqlite", { workspace, aiVisibilityDefault: ["coding_agent"] });
...
test("normal Node Wave 3 MCP path stays native-free", () => {
  ...
  const registration = serverSource.slice(serverSource.indexOf('server.registerTool("tasken.get_task_context"'), serverSource.indexOf('server.registerTool("tasken.get_note"'));
