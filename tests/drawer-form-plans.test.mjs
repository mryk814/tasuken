...
test("Capture form preserves long original text including surrounding whitespace on edit", () => {
  const text = " \n" + "研究🔬\nhttps://example.com/\n".repeat(400) + "末尾 \n";
  const result = plan(
    "capture_entry",
    [
  ...
  assert.equal(
    result.operations.find((operation) => operation.type === "capture_entry").entity.text,
    text,
  );
});
...
test("task form plan normalizes reminder, section, schedule, and Artifact Theme sync", () => {
  ...
  assert.equal(
    result.operations.some((operation) => operation.type === "schedule"),
    true,
  );
...
test("AI共通metadataは欄がある編集では保存され、欄が無い保存では消えない（#294）", () => {
  ...
  const supersededTask = superseded.operations.find(
...
test("出典は空行を捨て、Import由来のlocator情報を保つ（#294）", () => {
  ...
  assert.deepEqual(task.ai_source_refs[1], {
    kind: "url",
