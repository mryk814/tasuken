...
test("Sketchの自動保存は成功トーストを出さず、画面内の保存状態だけを更新する", () => {
  assert.match(
    page,
    /saveEntity\(\s*"sketch",\s*\{[\s\S]*?document,[\s\S]*?\},\s*\{ quiet: true \},?\s*\)/,
  );
...
test("Shape Arrow and Text remain active after creating an object", () => {
  const pointerUp =
    canvas.match(/function onPointerUp\([\s\S]*?function onPointerCancel/)?.[0] || "";
  const shapeBranch =
    pointerUp.match(/if \(tool === "shape"\)([\s\S]*?)if \(tool === "arrow"\)/)?.[1] || "";
  const arrowBranch =
    pointerUp.match(/if \(tool === "arrow"\)([\s\S]*?)function onPointerCancel/)?.[1] || "";
