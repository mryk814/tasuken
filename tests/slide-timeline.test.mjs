...
const timelineSource = readFileSync(
  "src/renderer/src/features/workspace/lib/slideTimeline.ts",
  "utf8",
);
...
test("unknown change_event types keep the Timeline projection alive with a fallback title", () => {
  ...
  assert.doesNotThrow(() =>
    timeline.buildSlideTimelineCandidates(domain, [], {
      themeId: "all",
      start: "2026-07-01",
      end: "2026-07-31",
    }),
  );
  const result = timeline.buildSlideTimelineCandidates(domain, [], {
    themeId: "all",
    start: "2026-07-01",
    end: "2026-07-31",
  });
...
test("16:9 SVG distinguishes task bars and activity diamonds and supports transparent background", () => {
  const candidates = timeline.buildSlideTimelineCandidates(domainFixture(), [], {
    themeId: "p1",
    start: "2026-07-01",
    end: "2026-07-31",
  });
