...
function readEnvelopes(inboxPath) {
  return fs
    ...
    .sort()
    .map((name) =>
      validateMcpProposalEnvelope(JSON.parse(fs.readFileSync(path.join(inboxPath, name), "utf8"))),
    );
}
...
test("legacy Task-work inbox envelope remains a compatibility reference for acceptance semantics", () => {
  ...
  try {
    ...
    for (const call of calls) {
      ...
      assert.deepEqual(
        { status: result.status, payload_type: result.payload_type },
        { status: "queued", payload_type: "task_work" },
      );
    ...
    for (const envelope of envelopes.filter((candidate) =>
      receiptActions.has(candidate.payload.task_work[0].action),
    )) {
    ...
    assert.equal(
      saved.some((entry) =>
        ["task", "work_receipt", "change_event", "reference"].includes(entry.type),
      ),
      false,
    );
...
test("seven content proposal tools retain exact create/merge payloads and stay proposal-only", () => {
  ...
  try {
    ...
    assert.ok(
      Buffer.byteLength(JSON.stringify(byTool.get("tasken.propose_note")), "utf8") > 64 * 1024,
    );
    assert.deepEqual(byTool.get("tasken.propose_note_edit").request.target, {
      type: "note",
      id: "note-1",
      base_version: 4,
    });
...
test("proposal validators enforce public bounds and reject private paths, credential URLs, and executable media", () => {
  ...
  try {
    assert.throws(
      () =>
        queueTool({
          ...
          payload: {
            task_work: [
              { action: "start", task_id: "task-1", expected_version: -1, caller: "Codex" },
            ],
          },
        }),
      /expected_version/,
            ...
            task_work: [
              {
                ...
                caller: "Codex",
                ...receiptFields,
                repository_context: { ...repositoryContext, cwd: "C:/private/tasuken" },
              },
            ],
          },
        }),
                ...
                external_references: [
                  {
                    ...
                    url: "https://user:pass@example.com/pr/1",
                  },
                ],
              },
            ],
    ...
    const oversized = envelope("notes", {
      notes: [{ action: "create", title: "Large", body: "x".repeat(1024 * 1024) }],
    });
    assert.throws(() => validateMcpProposalEnvelope(oversized), /1MB/);
    const normalized = validateMcpProposalEnvelope(
      envelope("notes", { notes: [{ action: "create", title: "Safe", body: "Body" }] }),
    );
...
test("all twelve proposal tools use their authenticated Core command owner", () => {
  ...
  for (const [index, name] of names.entries()) {
    ...
    const registrations = [...source.matchAll(marker)];
    assert.equal(registrations.length, 1, `${name} registration`);
...
test("Task-work Core client requires its named capability and rejects additive response fields", async () => {
  ...
  try {
    writeDiscovery([]);
    let fetchCalls = 0;
    const unavailable = new TaskenCoreClient({
      discoveryPath,
      fetch: async () => {
        fetchCalls += 1;
