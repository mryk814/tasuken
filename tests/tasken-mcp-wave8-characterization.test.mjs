...
function activityFixture() {
  ...
  const events = [
    ...
    activityEvent(
      "event-unsafe",
      "2026-08-20T15:34:00.000Z",
      {
        ...seed,
        canonical_refs: [
          {
            kind: "canonical_document",
            web_url: "https://user:pass@example.com/private?token=LEAK#fragment",
          },
        ],
      },
      {
        ...
        source_refs: [
          { kind: "url", locator: "https://user:pass@example.com/source?token=LEAK#fragment" },
        ],
...
test("Wave 8 legacy get_activity: timezone/date/order, visibility-before-limit, archive, and safe event projection", () => {
  ...
  try {
    ...
    assert.equal(
      ...
      JSON.stringify(result).replace(/user:pass|token=LEAK|Bearer LEAK|fragment|LEAK/g, ""),
    );
    assert.equal("token" in result.events[0].metadata, false);

    const ties = context.toolGetActivity({
      date: "2026-08-21",
      timezone: "Asia/Tokyo",
      sort_direction: "asc",
      limit: 100,
    ...
    assert.deepEqual(
      ties.events.map((event) => event.id),
      ["event-a", "event-b", "event-unsafe"],
    );
    assert.deepEqual(
      context
        .toolGetActivity({ date: "2026-08-20", timezone: "UTC", limit: 100 })
        .events.map((event) => event.id),
      ["event-prior", "event-a", "event-b", "event-unsafe"],
    );
    assert.deepEqual(
      context
        .toolGetActivity({
          from: "2026-08-20T15:30:00.000Z",
          to: "2026-08-20T15:30:00.000Z",
          limit: 100,
        })
        .events.map((event) => event.id),
      ["event-a", "event-b"],
    );
    // This legacy adapter uses the current shared projection: explicit kinds
    // override display hints, while hidden and archived entities stay excluded.
    assert.deepEqual(
      context
        .toolGetActivity({
          event_kinds: Array.from({ length: 21 }, () => "task_work_recorded"),
          limit: 100,
        })
        .events.map((event) => event.id),
      ["event-prior", "event-a", "event-b", "event-suppressed", "event-unsafe"],
    );

    const defaultLimit = context.toolGetActivity({}).events.length;
    assert.equal(defaultLimit, 4);
    assert.equal(
      context.toolGetActivity({ limit: 0 }).events.length,
      4,
      "legacy zero uses MAX_EVENTS via Number(limit) || MAX_EVENTS",
    );
    assert.equal(
      context.toolGetActivity({ limit: 501 }).events.length,
      4,
      "legacy max+1 clamps to MAX_EVENTS",
    );
    assert.equal(
      context.toolGetActivity({ limit: -1 }).events.length,
      1,
      "shared pagination clamps direct-method limits to at least one",
    );
    assert.equal(
      context.toolGetActivity({ timezone: "not/a-timezone", date: "2026-08-21", limit: 100 })
        .timezone,
      "Asia/Tokyo",
    );
    ...
    assert.equal(
      archived.events.find((event) => event.id === "event-archived").metadata.entity_status,
      "deleted",
    );
    assert.equal(
      context
        .toolGetActivity({ include_archived: false, limit: 100 })
        .events.some((event) => event.id === "event-archived"),
      false,
    );
  } finally {
    context.close();
...
test("Wave 8 legacy get_context_subgraph: bounded two-hop graph, cycles, suggested edges, hidden endpoints, and token limits", () => {
  ...
  try {
    ...
    assert.equal(
      result.nodes.some((node) => node.id === "task-suggested"),
      false,
    );
    assert.equal(
      result.nodes.some((node) => node.id === "note-hidden"),
      false,
    );
    ...
    const suggested = context.toolGetContextSubgraph({
      entity_type: "task",
      entity_id: "task-seed",
    ...
    const oneNode = context.toolGetContextSubgraph({
      entity_type: "task",
      entity_id: "task-seed",
      max_nodes: 1,
    });
    assert.deepEqual(
      oneNode.nodes.map((node) => node.id),
      ["task-seed"],
    );
    assert.equal(oneNode.truncated, true);
    assert.equal(
      context.toolGetContextSubgraph({ entity_type: "task", entity_id: "task-seed", max_edges: 0 })
        .edges.length,
      0,
    );
    assert.equal(
      context.toolGetContextSubgraph({
        entity_type: "task",
        entity_id: "task-seed",
        token_budget: 0,
      }).limits.tokenBudget,
    ...
    assert.deepEqual(
      context.toolGetContextSubgraph({ entity_type: "task", entity_id: "missing" }).exclusions,
      ["seed_not_found"],
    );
    assert.deepEqual(
      context.toolGetContextSubgraph({ entity_type: "", entity_id: "" }).exclusions,
      ["seed_not_found"],
    );

    const maximum = context.toolGetContextSubgraph({
      entity_type: "task",
      entity_id: "task-seed",
      max_hops: 3,
      max_nodes: 101,
    ...
    assert.deepEqual(maximum.limits, {
      maxHops: 2,
      maxNodes: 100,
      maxEdges: 200,
      maxDiagnostics: 48,
...
test("Wave 8 legacy export_ai_context: exact empty Markdown and bounded JSON scope semantics", () => {
  ...
  try {
    assert.equal(
      empty.toolExportAiContext({ format: "markdown" }),
      [
        "# Tasken Context",
        "",
  ...
  try {
    const json = context.toolExportAiContext({
      format: "json",
      scope: "open_items",
      max_items: 1,
      max_notes: 1,
    ...
    assert.equal(
      json.items.some((item) => item.id === "task-done"),
      false,
    );
    assert.equal(
      json.items.some((item) => item.id === "task-hidden"),
      false,
    );
    assert.deepEqual(
      json.notes.map((note) => note.id),
      ["note-visible"],
    );
    assert.equal(json.notes[0].body_markdown, undefined);
    assert.equal(json.notes[0].body_excerpt, "A very long ...");
    assert.deepEqual(
      json.knowledge_nodes.map((node) => node.id),
      ["knowledge-1"],
    );
    assert.deepEqual(
      json.repository_contexts.map((entry) => entry.id),
      ["repo-visible"],
    );
    ...
    assert.equal(
      JSON.stringify(withoutTimestamp).includes("user:pass"),
      true,
      "KNOWN UNSAFE LEGACY: resource URLs are returned raw by export_ai_context",
    );
    assert.equal(
      JSON.stringify(withoutTimestamp).includes("C:/private/resource.md"),
      true,
      "KNOWN UNSAFE LEGACY: resource local_path is returned raw by export_ai_context",
    );
    assert.equal(
      JSON.stringify(withoutTimestamp).includes("token=SECRET"),
      true,
      "KNOWN UNSAFE LEGACY: resource query credentials are returned raw by export_ai_context",
    );
    ...
    assert.equal(
      context.toolExportAiContext({ format: "json", max_items: 0 }).items.length,
      2,
      "legacy zero uses the default item limit",
    );
    assert.equal(
      context.toolExportAiContext({ format: "json", max_items: 101 }).items.length,
      2,
      "legacy max+1 clamps to 100",
    );
    assert.equal(
      context.toolExportAiContext({ format: "json", max_notes: -1 }).notes.length,
      1,
      "legacy negative uses the default note limit",
    );
    assert.equal(
      context.toolExportAiContext({ format: "json", scope: "selected_theme", theme_id: "missing" })
        .themes.length,
      0,
    );
  } finally {
