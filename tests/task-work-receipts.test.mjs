...
test("AI report stays needs_human_review and cannot complete before human accept", () => {
  ...
  const report = service.execute(envelope("ReportTaskDone", {
    taskId: "task-ai",
    receipt: {
      id: "receipt-ai-1",
      task_id: "task-ai",
...
test("MCP interim receipt keeps work in progress and only the done report enters human review", () => {
  ...
  const service = new ApplicationCommandService(repo);
  createAiTask(service);
  service.execute(envelope("StartTaskWork", { taskId: "task-ai" }, "start-sequence", [{ type: "task", id: "task-ai", version: 1 }]));
  ...
  const progress = service.execute(envelope("AppendWorkReceipt", {
    taskId: "task-ai",
    receipt: {
      id: "receipt-progress",
      task_id: "task-ai",
  ...
  }, "proposal-progress", [{ type: "task", id: "task-ai", version: 2 }]));
  ...
  const done = service.execute(envelope("ReportTaskDone", {
    taskId: "task-ai",
    receipt: {
      id: "receipt-done",
      task_id: "task-ai",
  ...
  }, "proposal-done", [{ type: "task", id: "task-ai", version: 3 }]));
  assert.equal(repo.get("task", "task-ai").work_state, "needs_human_review");
...
test("MCP blocker report appends a receipt, retains audit metadata, and moves work to blocked without completing Task", () => {
  ...
  const started = service.execute(envelope("StartTaskWork", { taskId: "task-ai", sourceSession: "proposal-start" }, "proposal-start", [{ type: "task", id: "task-ai", version: 1 }]));
  ...
  repo.save("ai_proposal", {
    ...
    source: "mcp",
    payload_type: "task_work",
    payload: { task_work: [{ action: "report_blocked", task_id: "task-ai", expected_version: 2, caller: "Codex" }] },
    request: {
  ...
  service.execute(envelope("ReportTaskBlocked", {
    taskId: "task-ai",
    receipt: {
      id: "receipt-blocked",
      task_id: "task-ai",
  ...
  }, "proposal-blocked", [{ type: "task", id: "task-ai", version: 2 }]));
  ...
  service.execute(envelope("StartTaskWork", { taskId: "task-ai" }, "resume-after-block", [{ type: "task", id: "task-ai", version: 3 }]));
  assert.equal(repo.get("task", "task-ai").work_state, "in_progress");
});
