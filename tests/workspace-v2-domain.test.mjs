...
test("legacy workspace conversion preserves first-class entity meanings", () => {
  ...
  assert.equal(v2.task_dependencies.length, 0);
  assert.equal(v2.plan_dependencies.length, 0);
  assert.equal(v2.references.length, 0);
...
test("inbox view sorts untriaged captures by newest timestamp", () => {
  ...
  assert.deepEqual(selectors.buildMicroMemoView(v2).entries.map((entry) => entry.id), ["cap-memo"]);
});
