...
test("mobile pairing persists only a token hash and revocation survives restart", () => {
  ...
  try {
    ...
    assert.deepEqual(paired.device.scopes, ["mobile:read", "mobile:task-write", "mobile:proposal-review"]);
    assert.throws(
      () => registry.pair({
        code: ticket.code,
    ...
    assert.deepEqual(stored.scopes, ["mobile:read", "mobile:task-write", "mobile:proposal-review"]);
