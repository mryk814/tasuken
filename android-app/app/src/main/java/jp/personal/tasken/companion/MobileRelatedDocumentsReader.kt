...
internal class MobileRelatedDocumentsReader(private val dao: MobileLocalDao, private val request: suspend (String) -> GatewayHttpResponse) {
    ...
    private suspend fun update(taskId: String, operation: suspend (String, RelatedDocumentsState) -> RelatedDocumentsState) = lock.withLock {
        ...
        val next = try { operation(serverId, previous) }
            ...
            accessRevoked = true
            RelatedDocumentsState(error = "接続権限が失効しました。端末の関連資料を削除しました。")
        }
    ...
    suspend fun refresh(taskId: String, nextPage: Boolean) = update(taskId) { serverId, previous ->
        val cursor = if (nextPage) previous.nextCursor ?: return@update previous else null
        ...
        previous.copy(documents = documents, bodies = previous.bodies.filter { body ->
    ...
    suspend fun load(taskId: String, type: String, id: String) = update(taskId) { serverId, previous ->
        ...
        if (data.status != "available") previous.copy(bodies = retained, documents = previous.documents.filterNot { it.type == type && it.id == id }, error = "参照先の削除または関連の解除を受信しました。")
        else {
            ...
            previous.copy(bodies = retained + CachedRelatedBody(type, id, body, Instant.now().toString()), error = null)
        }
    }
}
