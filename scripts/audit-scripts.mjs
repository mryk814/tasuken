...
const manualAllowlist = {
  ...
  "audit-rules.mjs":
    "Internal scanner module imported by audit-consistency; not a CLI entry point.",
  "core.mjs":
    "Internal architecture scanner imported by audit-architecture; not a CLI entry point.",
  "generate-katex-document-css.mjs":
    "Manual generated CSS refresh after upgrading KaTeX; generated output is committed.",
  "model-smoke.mjs":
    "Manual model/MCP integration check; it requires a real local database fixture.",
  "mcp-package-smoke-readiness.mjs":
    "Internal packaged MCP process readiness and diagnostic helper; imported by mcp-package-smoke.",
...
const workspaceRepository = readFileSync(
  path.join(root, "src/main/repositories/workspaceRepository.mjs"),
  "utf8",
);
const workspaceSchemaVersion = Number(
  workspaceRepository.match(/const SCHEMA_VERSION = (\d+)/)?.[1] || 0,
);
const findings = [];
...
export function inventoryEntry({
  ...
  workflowText,
  manualAllowlist: allowed = manualAllowlist,
}) {
  const base = path.basename(file);
  const packageOwners = Object.entries(scripts)
    .filter(([, command]) => command.includes(base))
    .map(([name]) => name);
  ...
  const contexts = packageOwners.flatMap((name) =>
    name.startsWith("release") || name === "package"
      ? ["release"]
      : name === "ci" || name.startsWith("test") || name.startsWith("audit") || name === "build"
        ? ["ci"]
        : ["manual"],
  );
  if (workflowReachable && !contexts.includes("ci") && !contexts.includes("release"))
    contexts.push("ci");
  ...
  return {
    ...
    purpose:
      allowlistedManual ||
      base.replace(/\.mjs$/, "") + " script; review package/workflow ownership before changing it.",
    lastRun: null,
    stale: !reachable,
  };
}
...
for (const [name, command] of Object.entries(packageScripts)) {
  for (const reference of command.matchAll(/(?:node\s+)?scripts[\\/]+([\w.-]+\.mjs)/g)) {
    ...
    if (!files.includes(file))
      findings.push({
        severity: "error",
        kind: "missing-package-target",
        script: name,
        file,
        message: "package script references a missing script file",
      });
  }
}

const entries = files.map((file) =>
  inventoryEntry({ file, packageScripts, workflowText: workflows }),
);
...
export const report = {
  ...
  ok:
    ...
    entries.every((entry) => !entry.stale || Boolean(manualAllowlist[path.basename(entry.file)])),
};
...
if (isMain) {
  if (process.argv.includes("--format=json")) console.log(JSON.stringify(report, null, 2));
    ...
    for (const finding of findings)
      console.log("[" + finding.severity + "] " + finding.message + ": " + finding.file);
  }
