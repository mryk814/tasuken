...
const routes = [
  {
    label: "Today",
    expected: ["実験室安全点検チェックシートを提出", "予測区間のcoverage低下を調べる", "継続中"],
  },
  { label: "ToDo", expected: ["候補バッチ #2を合成・焼成", "強度・導電率のPareto frontを再計算"] },
  { label: "Inbox", expected: ["炉位置の交絡？", "Alレビュー資料の単位", "XRD装置メモ"] },
  {
    label: "Timeline",
    expected: ["2026年度 LLZO探索サイクル", "Active learning cycle 2", "研究会 中間報告"],
  },
  {
    label: "Themes",
    navLabel: "All Themes",
    expected: ["Ta置換LLZO 固体電解質探索", "再生Al-Mg-Si 熱処理最適化", "個人業務"],
  },
  { label: "Notes", expected: ["GP-EI v4 モデル診断", "候補バッチ #2 実験計画"] },
  {
    label: "Knowledge",
    expected: [
      "L2407-Bの予測ずれは原料差か炉位置差か？",
      "ロットを跨ぐとGPの不確かさが過小評価される",
    ],
  },
  ...
  {
    label: "Artifacts",
    expected: ["screening-results.csv", "candidate-batch.json", "al-lot-ledger.xlsx"],
  },
  { label: "Agent Desk", expected: [] },
];
...
const failures = [];
const results = [];
const entityCounts = await page.evaluate(async () =>
  Object.fromEntries(
    await Promise.all(
      [
        "theme",
...
const hasUpdatedExperiment = await page.evaluate(async () =>
  (await window.api.entities.list("task")).some(
    (entry) => entry.title === "候補バッチ 2026-08-08 の焼成後密度を測定",
  ),
);
if (hasUpdatedExperiment) {
  routes
    .find((route) => route.label === "Today")
    ?.expected.push("候補バッチ 2026-08-08 の焼成後密度を測定");
  routes
    .find((route) => route.label === "ToDo")
    ?.expected.push("候補バッチ 2026-08-08 の焼成後密度を測定");
...
for (const [type, expected] of Object.entries(expectedCounts)) {
  if (entityCounts[type] < expected)
    failures.push(`${type}: ${entityCounts[type]}件（最低 ${expected}件）`);
}
...
for (const route of routes) {
  const button = page
    .locator(".sidebar button", { hasText: route.navLabel || route.label })
    .first();
  ...
  results.push({
    route: route.label,
    found: route.expected.length - missing.length,
    expected: route.expected.length,
    missing,
  });
  await page.screenshot({
    path: path.join(outputDirectory, `${route.label.toLowerCase().replaceAll(" ", "-")}.png`),
    fullPage: false,
  });
  if (route.label === "Notes") {
    ...
    if (!reportText.includes("Ta置換LLZO 週報 2026-W32"))
      failures.push("Notes: Reportフィルタに週報が表示されません。");
  }
}
...
if (!reloadedText.includes("Ta置換LLZO 固体電解質探索"))
  failures.push("再読み込み後にLLZO Themeを確認できません。");

console.log(
  JSON.stringify(
    {
      entityCounts,
      hasUpdatedExperiment,
      results,
      reloadPersisted: reloadedText.includes("Ta置換LLZO 固体電解質探索"),
      outputDirectory,
    },
    null,
    2,
