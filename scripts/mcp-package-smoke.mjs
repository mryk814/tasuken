...
} from "./mcp-package-smoke-readiness.mjs";

const executable = path.resolve(
  process.argv[2] || path.join("release", "win-unpacked", "Tasken.exe"),
);
...
const agentSessionHookPath = path.join(
  path.dirname(executable),
  "resources",
  "mcp",
  "agent-session-hook.mjs",
);
if (!fs.existsSync(agentSessionHookPath))
  throw new Error("Packaged Agent Session hook was not found.");
...
fs.writeFileSync(path.join(root, TASKEN_MCP_PACKAGE_SMOKE_MARKER_FILE), markerToken, {
  flag: "wx",
  mode: 0o600,
});
const desktopArgs = [
  `--user-data-dir=${root}`,
  "--mcp-package-smoke",
  `--mcp-package-smoke-marker=${markerToken}`,
];
...
try {
  ...
  for (const capability of TASKEN_MCP_REQUIRED_CORE_CAPABILITIES) {
    if (!core.capabilities.includes(capability))
      throw new Error(`Packaged Core capability missing: ${capability}`);
  }
  client = await connectMcp();
  const listed = await client.listTools();
  if (listed.tools.length !== 41)
    throw new Error(`Expected 41 packaged MCP tools, found ${listed.tools.length}.`);
  ...
  const item = read.structuredContent?.items?.find(
    (entry) => entry.id === "task-mcp-package-smoke",
  );
  if (
    !item ||
  ...
  if (
    restartedRead.structuredContent?.items?.filter((entry) => entry.id === "task-mcp-package-smoke")
      .length !== 1
  ) {
  ...
  if (
    verified.proposal_id !== proposalId ||
    verified.status !== "pending" ||
    verified.matching_count !== 1
  ) {
  ...
  for (const forbiddenPath of [fakeDbPath, legacyInboxPath]) {
    if (fs.existsSync(forbiddenPath))
      throw new Error(`Retired MCP persistence path was created: ${path.basename(forbiddenPath)}`);
  }
  if (!fs.existsSync(canonicalDbPath))
    throw new Error("Packaged Desktop canonical database was not created.");
