  ...
  const visit = (node) => {
    if (ts.isImportDeclaration(node) && ts.isStringLiteralLike(node.moduleSpecifier)) {
      ...
      add(
        ...
        node.moduleSpecifier,
        node.exportClause ? "re-export" : "export-all",
      );
    } else if (
  ...
  for (const node of sourceFile.statements) {
    if (ts.isExportDeclaration(node)) {
      if (node.exportClause && ts.isNamedExports(node.exportClause)) {
        ...
        exports.push(
          `*:${node.moduleSpecifier && ts.isStringLiteralLike(node.moduleSpecifier) ? node.moduleSpecifier.text : "local"}`,
        );
      }
      continue;
    }
    ...
    if (ts.isVariableStatement(node)) {
      for (const declaration of node.declarationList.declarations)
        exports.push(...bindingNames(declaration.name));
    } else if (
...
function loadTsconfigAliases(root, tsconfigPaths) {
  ...
  for (const configPath of tsconfigPaths || []) {
    ...
    const baseUrl = normalizePath(
      path.join(path.dirname(configPath), compilerOptions.baseUrl || "."),
    );
    for (const [pattern, targets] of Object.entries(compilerOptions.paths || {})) {
      for (const target of targets)
        aliases.push({ pattern, target: normalizePath(path.join(baseUrl, target)) });
    }
  }
  return aliases;
}
...
export function resolveImport({ sourceFile, specifier, fileSet, aliases = [] }) {
  ...
  if (specifier.startsWith(".")) {
    return (
      resolveSourcePath(path.join(path.dirname(sourceFile), specifier), fileSet) ||
...
function matchesExternal(target, names) {
  return names.some(
    (name) => target === `external:${name}` || target.startsWith(`external:${name}/`),
  );
}
...
function inspectImport(edge, sourceModule, targetModule, policy) {
  ...
  const sharedSource =
    sourceModule?.kind === "shared-contract" || sourceModule?.kind === "shared-kernel";
  const taskContractSource = sourceModule?.id === "shared.contracts.task";
  const taskSource = sourceModule?.id === "main.task";
  if (
    rendererSource &&
    (edge.target.startsWith("src/main/") ||
      edge.target.startsWith("src/preload/") ||
  ...
  if (
    ...
    (edge.target.startsWith("src/main/") ||
      edge.target.startsWith("src/preload/") ||
      edge.target.startsWith("src/renderer/") ||
      matchesExternal(edge.target, [
        "electron",
        "react",
        "zustand",
        "node:fs",
  ...
  if (
    taskContractSource &&
    matchesExternal(edge.target, ["electron", "react", "better-sqlite3", "sqlite3", "node:sqlite"])
  ) {
    findings.push(
      finding(
  ...
  if (
    ["shared.kernel", "shared.contracts.task", "main.task"].includes(targetModule?.id) &&
    sourceModule?.id !== targetModule?.id &&
    targetModule.publicEntrypoints?.length &&
    !isPublicTarget(edge.target, targetModule)
  ) {
    findings.push(
      finding(
  ...
  if (
    taskSource &&
    targetModule?.id === "main.application" &&
    /(?:workspaceService|snapshotService)/i.test(edge.target)
  ) {
  ...
  if (
    taskSource &&
    edge.target === "src/shared/types/workspace.ts" &&
    /\bWorkspaceData\b/.test(edge.specifier)
  ) {
    findings.push(
      finding(
        "task.legacy_workspace_snapshot",
  ...
  if (
    sourceModule?.kind === "renderer-feature" &&
    targetModule?.kind === "renderer-feature" &&
    sourceModule.id !== targetModule.id
  ) {
  ...
  if (
    edge.source.startsWith("tests/") &&
    targetModule?.publicEntrypoints?.length &&
    !isPublicTarget(edge.target, targetModule)
  ) {
    const owner = (policy.testOwnership || []).find(
      (entry) => normalizePath(entry.source) === edge.source,
    )?.module;
  ...
  if (
    ["main-application", "main-core"].includes(sourceModule?.kind) &&
  ...
  if (
    sourceModule?.kind === "main-infrastructure" &&
    (edge.target.startsWith("src/renderer/") || edge.target.startsWith("src/preload/"))
  ) {
    findings.push(
      finding(
  ...
  if (
    targetModule?.kind === "composition-root" &&
    sourceModule &&
    sourceModule.id !== targetModule.id &&
...
export function scanTextRules(file, source, policy) {
  ...
  const addMatches = (ruleId, pattern, reason, alternative, allowedPaths = []) => {
    if (
      allowedPaths.some(
        (allowed) =>
          file === allowed || file.startsWith(allowed.endsWith("/") ? allowed : allowed + "/"),
      )
    )
      return;
    lines.forEach((text, index) => {
      if (pattern.test(text))
        findings.push(finding(ruleId, file, index + 1, "", reason, alternative));
      pattern.lastIndex = 0;
    });
  };
...
function capabilitySurfaces(files, contents) {
  ...
  return surfaces.sort((left, right) =>
    `${left.file}|${left.global}`.localeCompare(`${right.file}|${right.global}`),
  );
}

function compareCapabilitySurfaces(current, baseline) {
  return current.map((surface) => {
    const previous = (baseline.surfaces || []).find(
      (entry) => entry.file === surface.file && entry.global === surface.global,
    );
...
function compatibilityInventory(files, contents, importsByFile, pairs) {
  ...
  for (const file of files) {
    const source = contents.get(file);
    if (/\b(?:entities|workspaceApi)\.(?:get|save|saveMany|remove|restore)\s*\(/.test(source))
      categories.get("generic-entity-api").add(file);
    if (hasGenericTaskWrite(source, file)) categories.get("generic-task-write").add(file);
    if (
      importsByFile
        .get(file)
        ?.some((entry) =>
          /(?:^|\/)workspaceService$/.test(entry.specifier.replace(/\.(?:ts|js|mjs)$/, "")),
        )
    )
      categories.get("workspace-service").add(file);
    if (
      importsByFile
        .get(file)
        ?.some((entry) => /(?:^|\/)workspaceRepository(?:\.mjs)?$/.test(entry.specifier))
    )
      categories.get("workspace-repository").add(file);
    if (
      /\b(?:workspaceApi\.load|window\.(?:api|researchDesk)\.workspace\.load|desktopApi\(\)\.workspace\.load)\s*\(/.test(
        source,
      )
    )
      categories.get("global-workspace-snapshot").add(file);
    const names = importedNames(source, file);
    if (names.has("ResearchDeskApi") || /interface\s+ResearchDeskApi\b/.test(source))
      categories.get("research-desk-api").add(file);
    if (names.has("WorkspaceData") || /interface\s+WorkspaceData\b/.test(source))
      categories.get("legacy-workspace-data").add(file);
    if (
      importsByFile
        .get(file)
        ?.some((entry) => /features\/workspace\/(?:lib|types)(?:\/|$)/.test(entry.target || ""))
    )
      categories.get("renderer-workspace-internals").add(file);
  }
 ...
 */
export function hasGenericTaskWrite(source, file = "source.ts") {
  if (!source) return false;
  if (/(?:saveEntity|saveWorkspaceEntity)\s*\(\s*["']task["']/.test(source)) return true;
  if (
    /(?:workspaceApi|entities|repository|workspaceRepository)\.(?:save|remove|restore)\s*\(\s*["']task["']/.test(
      source,
    )
  )
    return true;
  ...
  const containsTaskType = (node) => {
    if (
      ts.isPropertyAssignment(node) &&
      (ts.isIdentifier(node.name) || ts.isStringLiteral(node.name)) &&
      node.name.text === "type" &&
      ts.isStringLiteral(node.initializer) &&
      node.initializer.text === "task"
    )
      return true;
    return ts.forEachChild(node, containsTaskType) || false;
  };
  const visit = (node) => {
    if (ts.isCallExpression(node)) {
      ...
      const name = ts.isIdentifier(callee)
        ...
        : ts.isPropertyAccessExpression(callee)
          ? callee.name.text
          : null;
      if ((name === "saveMany" || name === "saveEntities") && node.arguments.some(containsTaskType))
        return true;
    }
  ...
  return files
    ...
    .map((file) => {
      ...
      return {
        ...
        consumers: [
          ...new Set(edges.filter((edge) => edge.target === file).map((edge) => edge.source)),
        ].sort(),
        runtimeDependencies: [
          ...new Set(edges.filter((edge) => edge.source === file).map((edge) => edge.target)),
        ].sort(),
        runtimeValidation:
          /(?:safeParse|\.parse\s*\(|\bz\.(?:object|union|enum|literal)\s*\()/.test(source),
...
function validateGeneratedSources(root, files, generatedPolicy) {
  const findings = [];
  const inventory = (generatedPolicy.entries || []).map((entry) => ({
    ...entry,
    path: normalizePath(entry.path),
  }));
  for (const entry of inventory) {
    const missing = ["path", "kind", "source", "version", "regenerationCommand", "owner"].filter(
      (key) => !entry[key],
    );
    if (missing.length) {
      findings.push(
        finding(
          "generated.invalid_manifest_entry",
          entry.path || "architecture/generated-sources.json",
          1,
          "",
          `Generated/vendor entry is missing: ${missing.join(", ")}.`,
          "Record path, kind, source, version, regeneration command, and owner.",
        ),
      );
    } else if (
      !files.includes(entry.path) &&
      !statSync(path.join(root, entry.path), { throwIfNoEntry: false })
    ) {
      findings.push(
        finding(
          "generated.missing_source",
          entry.path,
          1,
          "",
          "Generated/vendor manifest points to a missing file.",
          "Regenerate the file or remove the stale manifest entry.",
        ),
      );
    }
  }
  ...
  for (const file of files) {
    ...
    if (
      /\b(?:auto-generated|generated file|do not edit)\b/i.test(header) &&
      !registered.has(file)
    ) {
      findings.push(
        finding(
          "generated.unregistered_source",
          file,
          1,
          "",
          "A generated-source marker exists outside the generated/vendor manifest.",
          "Record provenance and the regeneration command in architecture/generated-sources.json.",
        ),
      );
    }
  }
...
function applySuppression(findingEntry, suppressions, today) {
  const matching = (suppressions.entries || []).find(
    (entry) =>
      entry.rule === findingEntry.ruleId &&
  ...
  const validDebt =
    matching.reason &&
    matching.owner &&
    Number.isInteger(matching.issue) &&
    (matching.expiresAt || matching.removalCondition);
  const expired = Boolean(matching.expiresAt && matching.expiresAt < today);
  return {
    ...findingEntry,
    suppressed: validDebt && !expired,
    suppression: { ...matching, validDebt: Boolean(validDebt), expired },
  };
}
...
export function analyzeArchitecture({
  ...
  const files = [...new Set(sourceRoots.flatMap((sourceRoot) => filesUnder(root, sourceRoot)))]
    .filter(
      (file) =>
        !fixtureRoots.some(
          (fixtureRoot) => file === fixtureRoot || file.startsWith(fixtureRoot + "/"),
        ),
    )
    .sort();
  ...
  for (const edge of edges)
    findings.push(
      ...inspectImport(
        edge,
        moduleFor(edge.source, modules),
        moduleFor(edge.target, modules),
        policy,
      ),
    );
  ...
  for (const module of modules) {
    if (module.kind === "renderer-feature" && module.publicEntrypoints.length === 0) {
      findings.push(
        finding(
          "module.missing_public_entrypoint",
          module.root,
          1,
          "",
          `Renderer feature '${module.id}' has no declared public entrypoint.`,
          `Create '${module.root}/public.ts' when the feature boundary is migrated.`,
        ),
      );
    }
  }
  ...
  for (const pair of pairs) {
    findings.push(
      finding(
        "contract.parallel_mjs_declaration",
        pair.implementation,
        1,
        pair.declaration,
        "Runtime .mjs and hand-maintained .d.mts declarations form a parallel type boundary.",
        "Move the public contract to checked TypeScript or register the pair as compatibility debt until migration.",
      ),
    );
  }
  const sharedOwnership = sharedOwnershipInventory(
    files,
    contents,
    edges,
    pairs,
  ...
  const capabilities = compareCapabilitySurfaces(
    capabilitySurfaces(files, contents),
    capabilityBaseline,
  );
    ...
    const moduleDependencies = [
      ...new Set(
        (importsByFile.get(file) || [])
          .map((edge) => moduleFor(edge.target, modules)?.id)
          .filter(Boolean),
      ),
    ].sort();
    const current = {
      ...
      imports: importCountFor(file, importsByFile),
      moduleDependencies: moduleDependencies.length,
    };
    ...
    return {
      ...
      increased: baseline
        ? current.lines > baseline.lines ||
          current.imports > baseline.imports ||
          current.moduleDependencies > baseline.moduleDependencies
        : true,
    };
  });
  ...
  for (const entry of compositionRoots.filter(
    (rootEntry) => rootEntry.increased && rootEntry.baseline,
  )) {
    findings.push(
      finding(
  ...
  const dependencies = edges
    .filter((edge) => !edge.target.startsWith("unresolved:"))
    .map((edge) => {
    ...
    .sort((left, right) =>
      [left.source, left.line, left.target]
        .join("|")
  ...
  return {
    ...
    summary: {
      ...
      newCompatibilityConsumers: compatibility.reduce(
      ...
      sharedExportedSymbols: sharedOwnership.reduce(
        (total, entry) => total + entry.exportedSymbols.length,
        0,
      ),
      unclassifiedSharedFiles: sharedOwnership.filter(
  ...
  return {
    compatibility: {
      ...
      categories: Object.fromEntries(
        report.compatibility.map((entry) => [entry.id, { consumerPaths: entry.currentPaths }]),
      ),
    },
    violations: {
      schemaVersion: 1,
      issue: 408,
      findings: report.findings.map((entry) => entry.fingerprint).sort(),
    },
    composition: {
      schemaVersion: 1,
      issue: 408,
      roots: report.compositionRoots.map(({ file, lines, imports, moduleDependencies }) => ({
        file,
        lines,
        imports,
        moduleDependencies,
    ...
    capabilities: {
      schemaVersion: 1,
      issue: 408,
      surfaces: report.capabilitySurfaces.map(({ file, global, properties }) => ({
        file,
        global,
        properties,
      })),
...
export function markdownReport(report) {
  const lines = [
    ...
    ...report.modules.map(
      (module) =>
        `| ${module.id} | ${module.kind} | ${module.status} | ${module.files} | ${module.publicEntrypoints.join("<br>") || "not declared"} |`,
    ),
    "",
    "## Compatibility debt",
    "",
    ...
    "|---|---:|---:|---:|---:|",
    ...report.compatibility.map(
      (entry) =>
        `| ${entry.id} | ${entry.currentPaths.length} | ${entry.baselinePaths.length} | ${entry.added.length} | ${entry.removed.length} |`,
    ),
    "",
    "## Composition roots",
    "",
    ...
    "|---|---:|---:|---:|---|",
    ...report.compositionRoots.map(
      (entry) =>
        `| ${entry.file} | ${entry.lines} | ${entry.imports} | ${entry.moduleDependencies} | ${entry.increased ? "yes" : "no"} |`,
    ),
    "",
    ...
    "|---|---|---:|---:|---:|",
    ...report.capabilitySurfaces.map(
      (entry) =>
        `| ${entry.file} | ${entry.global} | ${entry.propertyCount} | ${entry.added.length} | ${entry.removed.length} |`,
    ),
    "",
    "## Shared ownership",
    "",
    ...
    "|---|---:|---:|",
    ...[...new Set(report.sharedOwnership.map((entry) => entry.classification))]
      .sort()
      .map((classification) => {
        const entries = report.sharedOwnership.filter(
